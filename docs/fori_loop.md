...
res = fori_loop(upper, lower, body_fun, init)
```
- `upper`, `lower`: Loop boundaries.
- `body_fun`: User-defined function loop body operations.
...
[torch._higher_order_ops.while_loop](https://github.com/pytorch/pytorch/blob/ca6a0e1348ba7dcade1833d983b1b4ca12a5c1e1/torch/_higher_order_ops/while_loop.py#L69). 
...
res = while_loop(cond_fn, body_fn, init)
```
...
\
### Simple user guide
User could try these three simple test cases to better compare difference between `pure python for loop` and `fori_loop` and `while_loop`, these three test case have similar logic: cumulative plus 1 for ten times:
