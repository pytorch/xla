...
The SPMD programming model revolves around the concept of a "device mesh", also
called "mesh" for short. A device mesh is a logical N-dimensional arrangement of
...
- `device_ids` specifies the ordering of physical devices within the mesh in row
  ...
  The simplest ordering is `np.array(range(num_devices))`, but tweaking the
