...
found in the op lowering guide :ref:'op-lowering'.
...
PyTorch/XLA uses <https://github.com/pytorch/xla/issues/3560> to track the
status of codegen migration. When working on a codegen, please put your GitHub
alias with the PR link on the issue to avoid duplicate work.
...
characteristics: 1. Contains custom fallback code. For example in
\_adaptive_avg_pool3d, there is a conditional fallback:
...
2. Results in dynamic shape as these ops are WIP and may evolve over time. At
   some future point, we may bring the ops into codegen.
1. Does not invoke a tensor_method directly. For example:
...
`full_codegen` column and run `python setup.py install` under the xla directory
again. The build will fail (reason explained later in this guide) but you can
still see the generated file.
...
you might be running into a problem with PyTorch not yet having the necessary
implementation for the function to be generated. You can attempt to add the
...
Describing the generated code line by line: - Get and verify device from input
tensor
...
Wrap the newly created IR node in a XLATensor. And wrap the XLATensor within the
at::Tensor and return it as a result. Note that this part used to be manually
done in tensor_method.cpp.
...
A couple of things to keep in mind: - Codegen does not generate the `Clone`
method which is expected. There is no use of the `Clone` method even in
PyTorch/XLA today, we will remove them as part of the migration. - For every op,
it will generate a {OP}OutputShape method. We need to manually declare and
implement this method in a separate file. -For every op, it will generate a
Lower declaration. We need to manually implement this lowering function in a
separate file.
...
run the C++ test: 1. Build the xla through `python setup.py install` (note:
don't use the `BUILD_CPP_TESTS=0` flag since this will skip building the C++
tests) 2. Go into the `test/cpp/build` directory in your `pytorch/xla` 3. Run
the command to run the desired C++ test (for example, to run `Abs` C++ test):
