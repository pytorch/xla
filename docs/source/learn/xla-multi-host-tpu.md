...
<summary>
How to instal gcsfuse CLI on Linux/Ubuntu
</summary>
...
In this section we provide the python code to train a simple neural network model on MNIST dataset using SPMD computation model. We will run this code on multi-host TPU Pod, e.g. [v8-32](https://cloud.google.com/tpu/docs/v4).
...
The code snippet below contains the complete training script. Please save it into `mnist_xla.py` file. We will use this file in the next section to copy to TPU VM.
...
Let us give now details of the training script. As we will see below when we run it on multiple TPUs, we will sent over the SSH to all workers (e.g. to 8 worker if we are using v4-32) the same command to run: `python mnist_xla.py`.
...
Before calling the training method `train_mnist()` we have few lines of code to start the profiler:
```python
import torch_xla.debug.profiler as xp
...
Note: writing traces files from multiple workers to a shared folder (e.g. bucket mounted with `gcsfuse` as in our example) can present certain challenges. Namely, when multiple workers try create the same folder, `gcsfuse` can raise a "Permission denied" error. To avoid these issues we force workers to create subfolders and write traces into different folders:
```python
# output_folder should exist
output_folder = os.path.join(output_folder, f"proc_{host_id}")
```

We enable SPMD computation model with `xr.use_spmd()` and a sharded tensors will be split between the workers according to the defined meshes:
```python
...
The dataloader in this script is defined using `torch.utils.data.DataLoader` and the batches in the training step with shape `(B, 1, H, W)` are sharded across the batch dimension `B` on `D1 = num_devices // 2` devices. In other words each worker keeps a local shard of shape `(B // D1, 1, H, W)` of the loaded data to form a global tensor of the training data batch of the shape `(B, 1, H, W)`. The following code is responsible for this sharding:
```python
# Train step
data, target = data.to(device), target.to(device)
xs.mark_sharding(data, conv_mesh, ("data", "dim1", None, None))
