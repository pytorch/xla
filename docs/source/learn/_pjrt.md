...
[PJRT runtime](https://github.com/openxla/xla/tree/main/xla/pjrt) used by
...
- In XRT, all distributed workloads are multiprocess, with one process per
  ...
  [Multiprocessing section of the API guide](https://github.com/pytorch/xla/blob/master/API_GUIDE.md#running-on-multiple-xla-devices-with-multi-processing)
  ...
  - For other random number generation, use `torch.Generator` where possible.
    The global `torch` RNG is *not* thread-safe, even if you set the same
...
- Simple runtime configuration: just set `PJRT_DEVICE` to `TPU`, `CPU`, or
  `CUDA` and start using XLA! Or, let PJRT select a device automatically based
  on your environment.
...
- Better scaling: removes
  [XRT's limitation on parameter sizes](https://github.com/pytorch/xla/pull/3920)
  and supports up to 2048 TPU chips.
...
with host networking `--net=host` at this time. See the
[Cloud TPU documentation](https://cloud.google.com/tpu/docs/run-in-container)
...
You can also use `torchrun` to initiate the single-node multi-GPU training. For
example,
...
In the above example, `--nnodes` means how many machines (physical machines or
VMs) to be used (it is 1 since we do single-node training). `--nproc-per-node`
means how many GPU devices to be used.

### Multi-node GPU training

**Note that this feature only works for cuda 12+**. Similar to how PyTorch uses
multi-node training, you can run the command as below:
...
- `--node_rank`: the index of the current GPU machines. The value can be 0, 1,
  ..., ${NUMBER_GPU_VM}-1.
- `--nproc_per_node`: the number of GPU devices to be used on the current
  machine.
- `--rdzv_endpoint`: the endpoint of the GPU machine with node_rank==0, in the
  form `host:port`. The `host` will be the internal IP address. The `port` can
  be any available port on the machine. For single-node training/inference, this
  parameter can be omitted.

For example, if you want to train on 2 GPU machines: machine_0 and machine_1, on
the first GPU machine machine_0, run
...
the difference between the 2 commands above are `--node_rank` and potentially
`--nproc_per_node` if you want to use different number of GPU devices on each
machine. All the rest are identical. For more information about `torchrun`,
please refer to this [page](https://pytorch.org/docs/stable/elastic/run.html).
...
per TPU host. See the
[Cloud TPU documentation](https://cloud.google.com/tpu/docs/system-architecture-tpu-vm)
...
- Because the payload has to become part of the XLA graph, `torch_xla.sync()` is
...
consider using `` `torch.distributed.barrier ``
...
with a \]{.title-ref}[gloo]{.title-ref}\[ process group. If you are also using
the \]{.title-ref}[xla]{.title-ref}\[
...
device 0, which is *not* always worker 0. The `xla://` init_method finds this IP
automatically.
...
benefits vary significantly by task and model type, ranging from 0% to 175%. The
following chart shows the breakdown by task:
...
The PyTorch/XLA r2.0 release introduces support for the
[PJRT Plugin API](https://github.com/openxla/community/blob/main/rfcs/20230123-pjrt-plugin.md#rfc-openxla-pjrt-plugin),
