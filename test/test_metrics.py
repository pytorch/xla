...
class MetricsTest(unittest.TestCase):
  ...
  def test_get_fallback_ops(self):
    ...
    N = 10
    x = torch.rand(N, 1).to(torch.device('xla'))
    y = torch.rand(N, 1).to(torch.device('xla'))
    width = torch.rand(N, 1).to(torch.device('xla'))
    height = torch.rand(N, 1).to(torch.device('xla'))
    ...
    if not XLAExperimentalContains("nms"):
      ...
      import torchvision
      scores = torch.rand(N).to(torch.device('xla'))
