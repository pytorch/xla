...
def _init_with_reset_params(module):
  ...
  if is_meta:
    module.to_empty(device=torch_xla.device())
  with torch.no_grad():
...
class TestFSDPWithMetaDevice():
  ...
  def _test_simple_model_with_meta_device(self, meta_module_fn, init_fn=None):
    ...
    inp = torch.randn(10, 2, device=torch_xla.device())
    ...
    regular = MyModel(device=torch_xla.device())
  ...
  def test_simple_model_with_torchdistX_init_fn(self):
    ...
    def meta_module_fn():
      return deferred_init.deferred_init(MyModel, device=torch_xla.device())
