...
for name in "${test_names[@]}"; do
  ...
  if [ "$LOGFILE" != "" ]; then
    bazel $BAZEL_VERB $EXTRA_FLAGS //torch_xla/csrc/runtime:all //test/cpp:${name} --repo_env=HERMETIC_PYTHON_VERSION=3.10 --test_timeout 1200 ${FILTER:+"$FILTER"} 2> $LOGFILE
  else
    bazel $BAZEL_VERB $EXTRA_FLAGS //torch_xla/csrc/runtime:all //test/cpp:${name} --repo_env=HERMETIC_PYTHON_VERSION=3.10 --test_timeout 1200 ${FILTER:+"$FILTER"}
  fi
done
