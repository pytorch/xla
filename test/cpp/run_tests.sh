...
if [[ "$BAZEL_REMOTE_CACHE" == "1" ]]; then
  ...
  if [[ ! -z "$CACHE_SILO" ]]; then
    EXTRA_FLAGS="$EXTRA_FLAGS --remote_default_platform_properties='properties:{name:\"cache-silo-key\" value:\"$CACHE_SILO\"}'"
  fi
fi

if [ $BUILD_ONLY -eq 0 ]; then
  if [ "$LOGFILE" != "" ]; then
    bazel $BAZEL_VERB $EXTRA_FLAGS --test_output=all //third_party/xla_client:all //test/cpp:all ${FILTER:+"$FILTER"} |& tee $LOGFILE
  else 
