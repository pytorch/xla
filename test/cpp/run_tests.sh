...
if [ "$LOGFILE" != "" ]; then
  bazel $BAZEL_VERB $EXTRA_FLAGS //torch_xla/csrc/runtime:all //test/cpp:all --test_timeout 600 ${FILTER:+"$FILTER"} 2> $LOGFILE
else
  bazel $BAZEL_VERB $EXTRA_FLAGS //torch_xla/csrc/runtime:all //test/cpp:all --test_timeout 600 ${FILTER:+"$FILTER"}
fi
