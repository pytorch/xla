...
# Triton Pytorch/XLA-op registration
def ptxla_triton_add():
    output_shape_func = 
    out = triton_call(x, y, add_kernel, output_shape_func)
    return out
...
def test_triton_pytorchxla():
    ...
    output_torch = x + y
    print(output_torch)
    output_triton = ptxla_triton_add(x, y)
    print(output_triton)
    print(
...
def test_triton_pytorch():
    torch.manual_seed(0)
    size = 98432
    x = torch.rand(size, device='cuda')
    y = torch.rand(size, device='cuda')
    output_torch = x + y
    output_triton = pt_triton_add(x, y)
    print(output_torch)
    print(output_triton)
    print(
        f'The maximum difference between torch and triton is '
