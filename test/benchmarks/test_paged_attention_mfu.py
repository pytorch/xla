...
def _ref_jax_extended_paged_attention_static(
    q,  # [batch_size, query_len, num_query_heads, head_size]
    k_pages,  # [num_kv_heads, total_num_pages, page_size, head_size]
    v_pages,  # [num_kv_heads, total_num_pages, page_size, head_size]
    lengths,  # [batch_size], the effective kv_length.
  ...
  for i in range(batch_size):
    ...
    k = k_pages[:, indices]
    k = jnp.permute_dims(k, (1, 2, 0, 3))
    k = jnp.reshape(k, (-1, num_kv_heads, head_size))
    kv_len = k.shape[0]

    v = v_pages[:, indices]
    v = jnp.permute_dims(v, (1, 2, 0, 3))
    v = jnp.reshape(v, (-1, num_kv_heads, head_size))
...
# To remove the dynamism in _ref_jax_extended_paged_attention_static (otherwise, pl.estimate_cost throws an error "IndexError: Array slice indices must have static start/stop/step to be used with NumPy indexing syntax. Found slice(None, Traced<ShapedArray(int32[])>with<DynamicJaxprTrace>, None). To index a statically sized array at a dynamic position, try lax.dynamic_slice/dynamic_update_slice (JAX does not support dynamically sized arrays within JIT compiled functions)."), we can set up the test so that the effective_kv_len for each sequence to beh the max_kv_len, so that we can get rid of the dynamism such as `page_indices[i, :num_pages]`
def benchmark(args):
  dtype = jnp.bfloat16
  page_size = 16
  num_kv_heads = 1
  ...
  bs = len(kv_seq_lens_lst)
  num_q_heads = num_kv_heads * q_kv_head_ratio
  pages_per_sequence = (max_kv_len+page_size-1) // page_size
  # print(f'{q.dtype=}, {k_pages.dtype=}, {max_effective_kv_len=}, {page_indices.dtype=}, {effective_q_lens.dtype=}')
  cost = pl.estimate_cost(
    _ref_jax_extended_paged_attention_static,
    jax.ShapeDtypeStruct((bs, query_len, num_q_heads, head_dim), jnp.bfloat16),  # q
    jax.ShapeDtypeStruct((num_kv_heads, total_num_pages, page_size, head_dim), jnp.bfloat16),  # k
    jax.ShapeDtypeStruct((num_kv_heads, total_num_pages, page_size, head_dim), jnp.bfloat16),  # v
    jax.ShapeDtypeStruct((bs,), jnp.int32),  # max_effective_kv_len
    jax.ShapeDtypeStruct((bs, pages_per_sequence), jnp.int32),  # page_indices
    jax.ShapeDtypeStruct((bs,), jnp.int32),  # effective_q_lens
  )
  print('FLOPs:', cost.flops)
  print('Memory:', cost.bytes_accessed)
