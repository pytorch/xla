...
class UnboundedDynamismExportTest(unittest.TestCase):
    ...
    def test_add(self):
      ...
      constraints = [
          ...
          torch.export.dynamic_dim(args[0], 0) == torch.export.dynamic_dim(args[1], 0),
      ]
      ep = self._test_export_dynamism_wrapper(
          torch.ops.aten.add.Tensor, args, constraints
      )
    ...
    def test_addmm(self):
        ...
        ep = self._test_export_dynamism_wrapper(
            torch.ops.aten.addmm.default, args, constraints
        )
        shlo_module = exported_program_to_stablehlo(ep)
        shlo_text = shlo_module.get_stablehlo_text()
        self.assertTrue(
            "(%arg0: tensor<5xf32>, %arg1: tensor<5x5xf32>, %arg2: tensor<?x5xf32>) -> tensor<?x5xf32>"
            in shlo_text
        )
    ...
    def test_bmm(self):
        ...
        ep = self._test_export_dynamism_wrapper(
            torch.ops.aten.bmm.default, args, constraints
        )
        shlo_module = exported_program_to_stablehlo(ep)
        shlo_text = shlo_module.get_stablehlo_text()
        self.assertTrue(
            "(%arg0: tensor<?x64x197xf32>, %arg1: tensor<?x197x64xf32>) -> tensor<?x197x197xf32>"
            in shlo_text
        )
    ...
    def test_cat(self):
        ...
        ep = self._test_export_dynamism_wrapper(
            torch.ops.aten.cat.default, args, constraints
        )
        shlo_module = exported_program_to_stablehlo(ep)
        shlo_text = shlo_module.get_stablehlo_text()
        self.assertTrue(
    ...
    def test_conv(self):
        ...
        ep = self._test_export_dynamism_wrapper(
            torch.ops.aten.convolution.default, args, constraints
        )
        shlo_module = exported_program_to_stablehlo(ep)
        shlo_text = shlo_module.get_stablehlo_text()
        self.assertTrue(
            "(%arg0: tensor<5xf32>, %arg1: tensor<5x3x16x16xf32>, %arg2: tensor<?x3x224x224xf32>) -> tensor<?x5x14x14xf32>"
            in shlo_text
        )
    ...
    def test_div(self):
        ...
        ep = self._test_export_dynamism_wrapper(
            torch.ops.aten.div.Tensor, args, constraints
        )
        shlo_module = exported_program_to_stablehlo(ep)
        shlo_text = shlo_module.get_stablehlo_text()
        self.assertTrue(
            re.search(
                r'tensor<\?x12x197xf32>.*->.*tensor<\?x12x197xf32>',
                shlo_text) is not None)
    ...
    def test_gelu(self):
        ...
        ep = self._test_export_dynamism_wrapper(torch.ops.aten.gelu, args, constraints)
        shlo_module = exported_program_to_stablehlo(ep)
        # shlo_text = shlo_module.get_stablehlo_text()
        # self.assertTrue(
    ...
    def test_native_layer_norm(self):
        ...
        self.assertTrue(
            re.search(
                r"%arg.: tensor<\?x197x768xf32>.*->.*tensor<\?x197x768xf32>", shlo_text
            )
            is not None
        )
    ...
    def test_permute(self):
        ...
        ep = self._test_export_dynamism_wrapper(
            torch.ops.aten.permute.default, args, constraints
        )
        shlo_module = exported_program_to_stablehlo(ep)
        shlo_text = shlo_module.get_stablehlo_text()
        self.assertTrue(
            "(%arg0: tensor<?x197x12x64xf32>) -> tensor<?x12x197x64xf32>" in shlo_text
        )
    ...
    def test_select(self):
        ...
        ep = self._test_export_dynamism_wrapper(
            torch.ops.aten.select.int, args, constraints
        )
        shlo_module = exported_program_to_stablehlo(ep)
        shlo_text = shlo_module.get_stablehlo_text()
        self.assertTrue(
            "(%arg0: tensor<?x197x768xf32>) -> tensor<?x768xf32>" in shlo_text
        )
    ...
    def test_slice(self):
        ...
        ep = self._test_export_dynamism_wrapper(
            torch.ops.aten.slice.Tensor, args, constraints
        )
        shlo_module = exported_program_to_stablehlo(ep)
        shlo_text = shlo_module.get_stablehlo_text()
        self.assertTrue(
            "(%arg0: tensor<?x3x224x224xf32>) -> tensor<?x3x224x224xf32>" in shlo_text
        )
    ...
    def test_softmax(self):
        ...
        ep = self._test_export_dynamism_wrapper(
            torch.ops.aten._softmax.default, args, constraints
        )
        shlo_module = exported_program_to_stablehlo(ep)
        shlo_text = shlo_module.get_stablehlo_text()
        self.assertTrue(
            "(%arg0: tensor<?x12x197x197xf32>) -> tensor<?x12x197x197xf32>" in shlo_text
        )
