...
class KVCache(torch.nn.Module):
  ...
  def update_cache_with_hlfb(self, input_pos, k_val, v_val):
    ...
    # TODO(b/325465842): don't enable multiple outputs until bug is fixed.
...
class XlaMarkPatternTest(unittest.TestCase):
  ...
  def test_update_kv_cache(self):
    ...
    model.update_cache(input_pos, k_val, k_val)

    exported = torch.export.export(model, (input_pos, k_val, k_val))
