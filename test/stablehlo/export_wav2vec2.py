...
save_torch_module_as_tf_saved_model(model, args, tmp_dir, dynamic_shapes=dynamic_shapes)

# Verify numeric accuracy with an input with a different BS.
args = (torch.rand(2, 800),)
loaded_m = tf.saved_model.load(tmp_dir)
tf_input = pytree.tree_map_only(torch.Tensor, lambda x: tf.constant(x.numpy()), args)
