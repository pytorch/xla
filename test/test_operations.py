...
class TestAtenXlaTensor(XlaTestCase):
  ...
  def test_roll(self):
    ...
    self.assertEqual(
        single_roll,
        expected,
        message="{} did not equal expected result".format(single_roll))
    ...
    self.assertEqual(
        roll_backwards,
        expected,
        message="{} did not equal expected result".format(roll_backwards))

    data = numbers.view(2, 2, 2)
    rolled = data.roll(1, 0)
    expected = torch.tensor([5, 6, 7, 8, 1, 2, 3, 4],
                            device=device).view(2, 2, 2)
    self.assertEqual(
        expected,
        rolled,
        message="{} did not equal expected result: {}".format(rolled, expected))
    ...
    self.assertEqual(
        data,
        loop_rolled,
        message="{} did not equal the original: {}".format(loop_rolled, data))
    ...
    self.assertEqual(
        expected,
        data.roll(1),
        message="roll with no dims should flatten and roll.")
    self.assertEqual(
        expected,
        data.roll(1, dims=None),
        message="roll with no dims should flatten and roll.")
    ...
    self.assertEqual(
        double_rolled,
        expected,
        message="should be able to roll over two dimensions, got {}".format(
            double_rolled))

    # test roll over same dimension with repeat
    expected = torch.tensor([[5, 6, 7, 8], [1, 2, 3, 4]], device=device)
    repeated_dim_rolled = data.roll(shifts=(2, -1), dims=(0, 0))
    self.assertEqual(
        repeated_dim_rolled,
        expected,
        message="should be able to roll over two dimensions, got {}".format(
            repeated_dim_rolled))
    expected = torch.tensor([[7, 8, 5, 6], [3, 4, 1, 2]], device=device)
    mixed_repeated_dim_rolled = data.roll(shifts=(2, -2, 1), dims=(1, 0, 0))
    self.assertEqual(
        mixed_repeated_dim_rolled,
        expected,
        message="should be able to roll over two dimensions, got {}".format(
            mixed_repeated_dim_rolled))
    ...
    self.assertRaisesRegex(RuntimeError, "align",
    ...
    t[3] = True
    self.assertEqual(
        torch.tensor([False, True, False, False, True, False]), t.roll(1, 0))
