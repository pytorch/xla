...
def train_imagenet(FLAGS):
  ...
  if FLAGS.fake_data:
    ...
    train_dataset_len = (len(train_dataset.imgs)//global_batch_size)*global_batch_size
  ...
  def train_and_test(train_device_loader, test_device_loader):
    max_accuracy = 0.0
    epoch = 1
    ...
    for step, (data, target) in enumerate(train_device_loader):
      with xp.StepTrace('train_imagenet'):
        with xp.Trace('build_graph'):
          ...
          output = model(data)
          if FLAGS.lars:
              #output = (1 - FLAGS.label_smoothing) * torch.nn.functional.one_hot(target, num_classes=FLAGS.num_classes).float() + (FLAGS.label_smoothing / FLAGS.num_classes)
              pass
  ...
  def test_loop_fn(loader, epoch):
    xm.master_print('Epoch {} test begin {}'.format(epoch, test_utils.now()))
  ...
  max_accuracy = train_and_test(train_device_loader, test_device_loader)
