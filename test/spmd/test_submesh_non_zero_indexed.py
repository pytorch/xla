...
class SubmeshNonZeroIndexedTest(test_xla_sharding_base.XlaShardingTest):

  @classmethod
  def setUpClass(cls):
    super().setUpClass()

  def _create_non_zero_indexed_submesh_2dev(self):
    """Create 2-device submesh starting from non-zero index with no overlap"""
    if self.n_devices >= 4:
      device_ids = [2, 3]  # Use devices 2-3 when we have 4+ devices
    else:
      raise unittest.SkipTest(
          f"Need at least 4 devices for non-overlapping 2-device submesh, got {self.n_devices}"
      )
    mesh_shape = (1, 2)
    axis_names = ('x', 'y')
  ...
  @unittest.skipUnless(
      ...
      "Need at least 4 devices for non-zero-indexed 2-device submesh tests")
  @unittest.skipUnless(
  ...
  def test_pattern1_2dev(self):
    ...
    mesh = self._create_non_zero_indexed_submesh_2dev()
  ...
  def test_single_tensor_addition_2dev_direct_device(self):
    """Single tensor addition test with direct device creation and non-zero-indexed 2-device submesh"""
    mesh = self._create_non_zero_indexed_submesh_2dev()
  ...
  def test_complex_operations_direct_device(self):
    """Test complex tensor operations with direct device creation and non-zero-indexed submesh"""
    mesh = self._create_non_zero_indexed_submesh_2dev()
  ...
  def test_inplace_operations_direct_device(self):
    """Test in-place operations with direct device creation and non-zero-indexed submesh"""
    mesh = self._create_non_zero_indexed_submesh_2dev()
  ...
  def test_mesh_axis_names_length_mismatch(self):
    ...
    device_ids = [2, 3]  # Non-zero indexed devices
  ...
  def test_mesh_duplicate_device_ids(self):
    ...
    device_ids = [2, 2]  # Duplicate device IDs!
    mesh_shape = (1, 2)
  ...
  def test_mesh_device_ids_out_of_bounds(self):
    ...
    # Assuming we have 4 devices (0,1,2,3), use invalid IDs like 10,11
