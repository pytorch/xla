...
import torch_xla.experimental.megablox as mblx
...
XW_DONT_WANT_TO_USE_HYPOTHESIS = True
if CAN_USE_HYPOTHESIS and not XW_DONT_WANT_TO_USE_HYPOTHESIS:
  ...
  class GroupedMatmulTest(jtu.JaxTestCase):
    ...
    def gmm_test(
      ...
      lhs = random_dense((m, k), k1, lhs_dtype, limit=1)
      rhs = random_dense((num_groups, k, n), k2, rhs_dtype, limit=1)
      group_sizes = jnp.array([192, 256, 64], dtype=np.int32)
    ...
    def test_gmm(
      ...
      self.gmm_test(m, k, n, lhs_dtype, rhs_dtype, out_dtype, transpose_rhs)
