...
class DynamoInferenceBasicTest(unittest.TestCase):
  ...
  def test_resnet18(self):
    ...
    for data, _ in loader:
      ...
      self.assertTrue(
          torch.allclose(output_cpu, output.cpu(), rtol=1e-05, atol=1e-08))
...
class DynamoTrainingBasicTest(unittest.TestCase):
  ...
  def test_simple_model(self):
    ...
    self.assertTrue(torch.allclose(res_cpu, res_xla_dynamo.cpu()))
    self.assertTrue(torch.allclose(input.grad, xla_input.grad.cpu(), rtol=1e-05, atol=1e-08))
    # verifiy that tracing is skipped in following runs
    xla_input.grad = None
    met.clear_counters()
  ...
  def test_resnet18(self):
    ...
    for data, target in loader:
      ...
      self.assertTrue(
          torch.allclose(
              xla_output.cpu(), cpu_output.cpu(), rtol=1e-05, atol=1e-08))
...
class DynamoTrainingOptimizerTest(unittest.TestCase):
  ...
  def test_simple_model(self):
    ...
    for _ in range(5):
      ...
      assert torch.allclose(input.grad, xla_input.grad.cpu(), rtol=1e-05, atol=1e-08)
