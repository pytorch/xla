...
class DynamoInferenceBasicTest(unittest.TestCase):
  ...
  def test_resnet18(self):
    ...
    for data, _ in loader:
      ...
      self.assertTrue(
          torch.allclose(output_cpu, output.cpu(), rtol=1e-05, atol=1e-04))
...
class DynamoTrainingBasicTest(unittest.TestCase):
  ...
  def test_resnet18(self):
    ...
    for data, target in loader:
      ...
      self.assertTrue(
          torch.allclose(
              xla_output.cpu(), cpu_output.cpu(), rtol=1e-05, atol=1e-04))
...
class DynamoTrainingOptimizerTest(unittest.TestCase):
  ...
  def test_simple_model(self):
    ...
    for _ in range(5):
      ...
      assert torch.allclose(input.grad, xla_input.grad.cpu(), rtol=1e-05, atol=1e-04)
