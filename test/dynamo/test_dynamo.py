...
class DynamoTrainingBasicTest(unittest.TestCase):
  ...
  def test_simple_model(self):
    ...
    left = torch.norm(input.grad-xla_input.grad.cpu())
    print(left)
    testatol=1e-08
    testrtol=1e-05
    print("------------calculate like definition: right--------------")
    rightone = torch.mul(testrtol, torch.norm(xla_input.grad.cpu()))
    righttwo = testatol + rightone
    ...
    import numpy as np
    print(np.allclose(input.grad, xla_input.grad.cpu()))
    print("------------run: torch.allclose(input.grad, xla_input.grad.cpu())) --------------")
    print(torch.allclose(input.grad, xla_input.grad.cpu()))
    print("------------run: torch.isclose(input.grad, xla_input.grad.cpu())) --------------")
    print(torch.isclose(input.grad, xla_input.grad.cpu()))
    print("------------checked !!!!!!!!!!!!!!!!---------------")
