...
def compile_step_func(step, weights, buffers, opt_state, args, label, mesh):
  step, weights, buffers, opt_state, args, label = interop.jax_view((step, weights, buffers, opt_state, args, label))
  wshardings = tree_map(lambda a: a.sharding if isinstance(a, jax.Array) else None,
                       weights)
  bshardings = tree_map(lambda a: a.sharding if isinstance(a, jax.Array) else None,
                       buffers)
  ...
  lowered = jax.jit(
    ...
    out_shardings=(NamedSharding(mesh, P()), wshardings, oshardings),
      ...
      weights, buffers, opt_state, args, label
  )
