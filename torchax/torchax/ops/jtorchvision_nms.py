...
def _bbox_overlap(boxes, gt_boxes):
    ...
    i_area = jnp.maximum((i_xmax - i_xmin), 0) * jnp.maximum(
        (i_ymax - i_ymin), 0
    )
...
def _cross_suppression(in_args):
    ...
    new_slice = lax.dynamic_slice(
        boxes,
        [0, inner_idx * _NMS_TILE_SIZE, 0],
        [batch_size, _NMS_TILE_SIZE, 4],
    )
    iou = _bbox_overlap(new_slice, box_slice)
    ret_slice = (
        jnp.expand_dims(
            (jnp.all(iou < iou_threshold, [1])).astype(box_slice.dtype), 2
        )
        * box_slice
    )
...
def _suppression_loop_body(in_args):
    ...
    box_slice *= jnp.expand_dims(
        1.0 - suppressed_box.astype(box_slice.dtype), 2
    )
...
def non_max_suppression_padded(scores, boxes, max_output_size, iou_threshold):
    ...
    pad = (
        int(jnp.ceil(float(num_boxes) / _NMS_TILE_SIZE)) * _NMS_TILE_SIZE
        - num_boxes
    )
...
def nms(boxes, scores, iou_threshold):
    ...
    res = non_max_suppression_padded(
        scores, boxes, max_output_size, iou_threshold
    )
    return res
