...
"""
Copied from https://github.com/pytorch/xla/blob/master/torch_xla/experimental/pallas_kernels/ragged_paged_attention_v2.py
since the pypi torchxla package does not have the updated function.
"""
...
def static_validate_inputs(
  ...
  if (num_kv_pages_per_block is not None and
      not 0 < num_kv_pages_per_block <= pages_per_seq):
    raise ValueError(
    ...
    def compute_with_kv_blk_in_cur_seq(kv_states):
      ...
      for kv_head_idx in range(num_kv_heads_per_blk):
        ...
        q = fold_on_2nd_minor(q_ref[:, q_head_idx:q_head_idx +
                                    num_q_heads_per_kv_head, :])
        k, v = strided_load_kv(kv_ref, kv_head_idx * 2,
