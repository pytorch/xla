...
def _aten_complex(real, imag):
    ...
    """
    return jnp.array(real, dtype=jnp.float32) + 1j * jnp.array(
...
def _aten_searchsorted(sorted_sequence, values):
    ...
    if sorted_sequence.dtype == np.dtype(
        np.int32
    ) or sorted_sequence.dtype == np.dtype(np.int32):
        # res = res.astype(new_dtype)
...
def repeat_interleave(self, repeats, dim=0):
    ...
    return jnp.repeat(
        self, repeats, dim, total_repeat_length=total_repeat_length
    )
...
def _aten_cummax(x, dim):
    ...
    res = jax.lax.associative_scan(
        cummax_reduce_func, {"val": x, "idx": indices}, axis=axis
    )
...
def _aten_cummin(x, dim):
    ...
    res = jax.lax.associative_scan(
        cummin_reduce_func, {"val": x, "idx": indices}, axis=axis
    )
    return res["val"], res["idx"]
...
def _aten_bucketize(
    input, boundaries, *, out_int32=False, right=False, out=None
):
...
def _aten_convolution(
    ...
    if transposed:
        ...
        res = jax.lax.conv_general_dilated(
            ...
            dimension_numbers=create_default_conv_dimension_numbers(
                len(stride)
            ),
...
def _aten_mode(input, dim=-1, keepdim=False, *, out=None):
    ...
    dim = (
        input.ndim + dim
    ) % input.ndim  # jnp.scipy.stats.mode does not accept -1 as dim
...
def _aten_native_group_norm(input, weight, bias, N, C, HxW, group, eps=1e-5):
    ...
    if weight is not None and bias is not None:
        output = bias.reshape(affine_shape) + output * weight.reshape(
            affine_shape
        )
...
def _aten_linalg_vector_norm(self, ord=2, dim=None, keepdim=False, dtype=None):
    ...
    if ord not in {2, float("inf"), float("-inf"), "fro"} and not isinstance(
        ord, (int, float)
    ):
        raise ValueError(
    ...
    for i in range(len(index_shape)):
        ...
        if i == dim:
            ...
            input_indexes.append(
                jnp.broadcast_to(
                    jnp.arange(index_shape[i]).reshape(target_shape),
                    index_shape,
                )
            )
...
def _aten_broadcast_tensors(*tensors):
    ...
    if (
        len(tensors) == 1
        and len(tensors[0]) >= 1
        and isinstance(tensors[0][0], jax.Array)
    ):
        tensors = tensors[0]
    ...
    broadcasted_tensors = [
        jax.lax.broadcast_in_dim(
            t, output_shape, _broadcast_dimensions(t.shape, output_shape)
        )
        for t in tensors
    ]
...
def _aten_constant_pad_nd(input, padding, value=0):
    ...
    return jax.lax.pad(
        input, padding_value=value_casted, padding_config=pad_dim
    )
...
def diag_indices_with_offset(input_shape, offset, dim1=0, dim2=1):
    ...
    if dim1 == dim2 or not (0 <= dim1 < input_len and 0 <= dim2 < input_len):
        raise ValueError(
            "dim1 and dim2 must be different and in range [0, "
            + str(input_len - 1)
            + "]"
        )
...
def _aten_linalg_lstsq(A, B, rcond=None, driver="gelsy"):
    ...
    if is_batched:
        ...
        if driver in ["gelsd", "gelss"]:
            singular_values = singular_values.reshape(
        ...
        X, residuals, rank, singular_values = jnp.linalg.lstsq(
            A, B, rcond=rcond
        )
...
def _aten_linalg_lu_solve(LU, pivots, B, left=True, adjoint=False):
    ...
    if not left:
        ...
        x = jax.scipy.linalg.lu_solve(
            (LU, pivots), jnp.matrix_transpose(B), trans
        )
...
def _aten_nonzero_static(input, size, fill_value=-1):
    ...
    if size < indices.shape[0]:
        ...
        padding = jnp.full(
            (size - indices.shape[0], indices.shape[1]),
            fill_value,
            dtype=indices.dtype,
        )
...
def _aten_nonzero(x, as_tuple=False):
    ...
    if (
        jnp.ndim(x) == 0
    ):  # when x is scalar, return torch.tensor([], size=(1, 0), dtype=torch.int64)
...
def _aten_unique_dim(
    input_tensor, dim, sort=True, return_inverse=False, return_counts=False
):
...
def _aten_unique2(
    input_tensor, sort=True, return_inverse=False, return_counts=False
):
...
def _aten_unique_consecutive(
    input_tensor, return_inverse=False, return_counts=None, dim=None
):
    ...
    if return_inverse or return_counts:
        ...
        inverse = (
            jnp.reshape(
                jnp.repeat(jnp.arange(len(counts)), counts), inverse_shape
            )
            if return_inverse
            else None
        )
    ...
    for i, idx in enumerate(indices.flatten()):
        ...
        for y in range(in_y, in_y + kH):
            for x in range(in_x, in_x + kW):
                if (
                    0 <= y < grad_input.shape[2]
                    and 0 <= x < grad_input.shape[3]
                ):
                    grad_input = grad_input.at[y, x].add(
                        grad_output.flatten()[i]
                    )
...
def _aten_uniform(self, from_=0, to=1, *, generator=None, env=None):
    assert from_ <= to, (
        f"Uniform from(passed in {from_}) must be less than to(passed in {to})"
    )
    shape = self.shape
    ...
    def vectorized(x, n_i):
        ...
        def default(x):
            def f(k, carry):
                ...
                return (
                    q,
                    ((k * 2 + (jnp.ones_like(x) - x)) * q - k * p) / (k + 1),
                )

            _, q = jax.lax.fori_loop(
                1, n_i, f, init_val=(1.0, jnp.ones_like(x) - x)
            )
            return q
...
def _aten_special_modified_bessel_i1(self):
    ...
    def default(x):
        ...
        return jax.lax.cond(
            x < 0,
            lambda: -(
                jnp.exp(jnp.abs(x)) * (0.5 * (b - p)) / jnp.sqrt(jnp.abs(x))
            ),
            lambda: jnp.exp(jnp.abs(x))
            * (0.5 * (b - p))
            / jnp.sqrt(jnp.abs(x)),
        )
...
def _aten_special_modified_bessel_k0(self):
    ...
    def small(x):
        ...
        return 0.5 * (a - p) - jnp.log(
            0.5 * x
        ) * _aten_special_modified_bessel_i0(x)
...
def _aten_special_modified_bessel_k1(self):
    ...
    def small(x):
        ...
        return (
            jnp.log(0.5 * x) * _aten_special_modified_bessel_i1(x)
            + 0.5 * (a - p) / x
        )
...
def _aten_special_bessel_j0(self):
    ...
    def small(x):
        ...
        rp = op_base.foreach_loop(
            RP, lambda carry, rp_i: carry * (x * x) + rp_i
        )
        rq = op_base.foreach_loop(
            RQ, lambda carry, rq_i: carry * (x * x) + rq_i
        )
...
def _aten_special_bessel_y0(self):
    ...
    def small(x):
        ...
        yp = op_base.foreach_loop(
            YP, lambda carry, yp_i: carry * (x * x) + yp_i
        )
        yq = op_base.foreach_loop(
            YQ, lambda carry, yq_i: carry * (x * x) + yq_i
        )
...
def _aten_special_chebyshev_polynomial_t(self, n):
    ...
    def vectorized(x, n_i):
        ...
        def one_x(x):
            return jnp.where(
                (x > 0) | (n_i % 2 == 0), jnp.ones_like(x), -jnp.ones_like(x)
            )
...
def _aten_multinomial(
    input, num_samples, replacement=False, *, generator=None, out=None, env=None
):
    assert num_samples <= input.shape[-1] or replacement, (
...
def _aten_linalg_solve_triangular(
...
def _aten_median(self, dim=None, keepdim=False):
    ...
    if dim is None:
        ...
        index = _with_reduction_scalar(
            _get_median_index, self, dim, keepdim
        ).astype(jnp.int64)
        return output, index
...
def _aten_nanmedian(input, dim=None, keepdim=False, *, out=None):
    ...
    if dim is None:
        ...
        index = _with_reduction_scalar(
            _get_median_index, input, dim, keepdim
        ).astype(jnp.int64)
        return output, index
...
def _aten_triangular_solve(
    b, a, upper=True, transpose=False, unittriangular=False
):
    return (
...
def _aten_trilinear(
    i1, i2, i3, expand1, expand2, expand3, sumdim, unroll_dim=1
):
    return _aten_sum(
        jnp.expand_dims(i1, expand1)
        * jnp.expand_dims(i2, expand2)
        * jnp.expand_dims(i3, expand3),
        sumdim,
    )
...
mutation_ops_to_functional = {
    ...
    torch.ops.aten.bernoulli_.float: op_base.InplaceOp(
        _aten_bernoulli, is_jax_func=True
    ),
    ...
    torch.ops.aten.scatter_reduce_.two: op_base.InplaceOp(
