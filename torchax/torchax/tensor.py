...
class Tensor(torch.Tensor):
    ...
    def flatten(self, start_dim=0, end_dim=-1):
        if end_dim == -1:
            end_dim = self.ndim
        new_shape = (
            self._elem.shape[:start_dim]
            + (-1,)
            + self._elem.shape[end_dim + 1 :]
        )
...
def _make_debug_msg(is_dispatch, log_args, func, args, kwargs):
    ...
    args_msg = (
        "args: " + ",".join(_display(a) for a in args) if log_args else ""
    )
    kwargs_msg = (
        "kwargs: "
...
class Environment(contextlib.ContextDecorator):
    ...
    def get_as_jax_device(self, device: Any):
        ...
        if (
            not self.config.use_torch_native_for_cpu_tensor
        ...
        if isinstance(the_tensor, Tensor):
            ...
            if new_device is not None:
                match str(new_device).lower():
                    ...
                    case _:
                        logging.error(
                            f"torchax.Tenosr cannot handle device {new_device}"
                        )
    ...
    def get_and_rotate_prng_key(
        ...
        if generator is not None:
            with mode_utils.no_dispatch(), torch._C.DisableTorchFunction():
                self._prng_key[...] = jax.random.key(
                    generator.initial_seed() % (2**63)
                )
    ...
    def j2t_copy(self, args):
        ...
        return torch_pytree.tree_map_only(
            jax.Array,
            lambda x: mappings.j2t(
                x, self.config.use_dlpack_for_data_conversion
            ),
            args,
        )
