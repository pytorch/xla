...
def extract_avals(exported):
    ...
    def _to_aval(arg_meta, symbolic_shapes):
        ...
        return jax.ShapeDtypeStruct(
            shape, mappings.t2j_dtype(tensor_meta.dtype)
        )

    def _get_inputs(exported):
        """Return placeholders with input metadata"""
        placeholders = [
            p for p in exported.graph.nodes if p.op == "placeholder"
        ]
        input_placeholders = [
            p
    ...
    def _build_symbolic_shapes(range_constraints):
        ...
        def _build_symbolic_constraints(symbol_name, torch_constraint):
            ...
            return tuple(
                sympy.pretty(c, use_unicode=False) for c in constraints
            )
        ...
        for sym, constraint in symbol_variables + symbol_exprs:
            symbolic_shape = _build_symbolic_shape(
                sym, constraint, symbolic_shapes
            )
    ...
    if DEBUG:
        ...
        for arg in args:
            print(
                ...
                arg.meta,
                "--> ",
                _to_aval(arg.meta, symbolic_shapes),
            )
