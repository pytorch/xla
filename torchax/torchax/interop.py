...
class JittableModule(torch.nn.Module):
    def __init__(
        self, m: torch.nn.Module, extra_jit_args={}, dedup_parameters=True
    ):
        super().__init__()
...
class CompileMixin:
    ...
    def jit(self, method):
        ...
        def call(*args, **kwargs):
            return jitted(
                self.named_paramters(), self.named_buffers(), *args, **kwargs
            )
...
def _jax_view(t: TorchValue) -> JaxValue:
    ...
    if isinstance(t, torch.Tensor):
        assert isinstance(t, tensor.Tensor) or isinstance(t, tensor.View), type(
            t
        )
        return t.jax()
    ...
    if not isinstance(t, torch.nn.Module) and callable(
        t
    ):  # t is a TorchCallable
    ...
    def inner(*args, **kwargs):
        ...
        class JaxFun(torch.autograd.Function):
            ...
            def forward(ctx, tree_def, *flat_args_kwargs):
                ...
                y, fun_vjp = call_jax(
                    _jax_forward, fn, other, tree_def, tensors
                )
...
def jax_jit(
    torch_function, kwargs_for_jax_jit=None, fix_for_buffer_donation=False
):
    return wrap_jax_jit(
