...
def randomize_input(inputs):
    if isinstance(inputs, torch.Tensor):
        ...
        logger.warning(f"randomize_input can not handle input of type {type(inputs)}")
        return inputs
...
def get_cpu_name():
    return (
        subprocess.check_output(["lscpu"], encoding="utf-8")
        .split("Model name:")[1]
        .split("\n")[0]
        .strip()
    )
...
def get_gpu_name():
    return subprocess.check_output(
        ["nvidia-smi", "--query-gpu=gpu_name", "--format=csv"], encoding="utf-8"
    ).split("\n")[1]
