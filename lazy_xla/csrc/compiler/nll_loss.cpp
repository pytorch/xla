...
xla::XlaOp OneHotIota(xla::XlaBuilder* builder, xla::int64 depth, int axis,
  ...
  if (ignore_index >= 0 && ignore_index < depth) {
    xla::XlaOp ignore_index_op =
        xla::Broadcast(compiler::XlaHelpers::ScalarValue<xla::int64>(
...
xla::XlaOp LabelsToOneHot(xla::XlaBuilder* builder, xla::int64 depth, int axis,
  ...
  const xla::Shape& indices_shape = compiler::XlaHelpers::ShapeOfXlaOp(indices);
...
WeightScale GetMaskedWeight(xla::XlaOp weight, const xla::Shape& logits_shape,
  ...
  const xla::Shape& labels_shape = compiler::XlaHelpers::ShapeOfXlaOp(labels);
  xla::XlaOp valid_bitmap = xla::Ne(
      labels, compiler::XlaHelpers::ScalarValue<xla::int64>(
  ...
  if (!weight.IsUninitialized()) {
    ...
    xweight = compiler::XlaHelpers::ScalarBroadcast<float>(1.0, logits_shape,
                                                           labels.builder());
  }
  xla::XlaOp zeros = compiler::XlaHelpers::ScalarBroadcast<float>(
      0.0, logits_shape, labels.builder());
...
xla::XlaOp BuildNllLoss(xla::XlaOp logits, xla::XlaOp labels, xla::XlaOp weight,
  ...
  // Handle the NaN generated by mul(inf/-inf, 0).
  mul = xla::Select(xla::Ne(mul, mul),
                    xla::Broadcast(zero, logits_shape.dimensions()), mul);
