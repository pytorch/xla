...
lazy_tensors::PrimitiveType XlaTypeFromTensorType(at::ScalarType scalar_type,
                                                  const Device& device) {
  switch (scalar_type) {
...
void TensorToBuffer(const at::Tensor& tensor, const xla::Shape& dest_shape,
  ...
  xla::Shape src_shape = compiler::XlaHelpers::XlaShape(MakeTorchTensorLayout(
...
void PopulateTensorBuffer(const at::Tensor& tensor,
                          const lazy_tensors::client::ShapeData& dest_shape,
                          void* dest_buffer, size_t dest_buffer_size,
                          const Device& device) {
  PopulateTensorBuffer(
      tensor, compiler::XlaHelpers::XlaShape(lazy_tensors::Shape(dest_shape)),
      dest_buffer, dest_buffer_size, device);
}
...
xla::ComputationClient::DataPtr TensorToXlaData(const at::Tensor& tensor,
  ...
  source_tensors.emplace_back(
      lazy_tensors::ToShapeData(compiler::XlaHelpers::LazyTensorsShape(shape)),
...
at::Tensor XlaLiteralToTensor(const xla::Literal& literal,
  ...
  xla::Shape torch_shape = compiler::XlaHelpers::XlaShape(MakeTorchTensorLayout(
      literal.shape().dimensions(), /*dynamic_dimensions=*/{},
      xla::ComputationClient::LazyTensorPrimitiveType(
...
std::vector<xla::ComputationClient::DataPtr> CreateTensorsData(
  ...
  for (size_t i = 0; i < tensors.size(); ++i) {
    ...
    source_tensors.emplace_back(
        lazy_tensors::ToShapeData(
            compiler::XlaHelpers::LazyTensorsShape(std::move(shape))),
        devices[i], std::move(populate_fn));
  }
...
xla::Literal GetTensorLiteral(const at::Tensor& tensor, const xla::Shape* shape,
  ...
  if (shape == nullptr) {
    ...
    computed_shape = compiler::XlaHelpers::XlaShape(MakeTorchTensorLayout(
...
xla::Shape MakeShapeWithDeviceLayout(const xla::Shape& shape,
  ...
  xla::ShapeUtil::ForEachMutableSubshape(
      &device_shape, [&](xla::Shape* subshape, const xla::ShapeIndex&) {
        if (subshape->IsArray()) {
          *subshape =
              compiler::XlaHelpers::XlaShape(MakeArrayShapeFromDimensions(
...
xla::Shape CreateComputationShapeFromTensor(const at::Tensor& tensor,
                                            const Device* device) {
  Device xla_device = GetDeviceOrCurrent(device);
  return compiler::XlaHelpers::XlaShape(MakeArrayShapeFromDimensions(
