...
bool IsF32BatchNormWithFP16Inputs(const xla::XlaOp& input,
  ...
  if (compiler::XlaHelpers::ShapeOfXlaOp(input).element_type() ==
          xla::PrimitiveType::F16 &&
      compiler::XlaHelpers::ShapeOfXlaOp(weight).element_type() ==
...
xla::XlaOp VarianceRecover(xla::XlaOp invstd, float eps_value) {
  const xla::Shape& invstd_shape = compiler::XlaHelpers::ShapeOfXlaOp(invstd);
  xla::XlaOp eps = compiler::XlaHelpers::ScalarValue(
...
xla::XlaOp BatchNormVarianceInvert(xla::XlaOp variance, float eps_value) {
  const xla::Shape& variance_shape =
      compiler::XlaHelpers::ShapeOfXlaOp(variance);
  xla::XlaOp eps = compiler::XlaHelpers::ScalarValue(
