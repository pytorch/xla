...
struct ReductionInfo {
  ...
  compiler::XlaHelpers::DynamicSize element_count;
};
...
ReductionInfo GetReductionInfo(xla::XlaOp input, const xla::Shape& shape,
  ...
  rinfo.element_count =
      compiler::XlaHelpers::GetDimensionsSize({input}, dimensions);
  return rinfo;
}
...
xla::XlaOp GetScaleValue(xla::XlaOp input, xla::XlaOp count,
  ...
  xla::XlaOp zero =
      xla::Zero(input.builder(), compiler::XlaHelpers::TypeOfXlaOp(count));
...
xla::XlaOp AverageValue(xla::XlaOp input, xla::XlaOp reduced) {
  ...
  xla::XlaOp num_elements =
      compiler::XlaHelpers::GetDimensionsSize(
          {input}, compiler::XlaHelpers::GetAllDimensions(input_shape))
          .size;
...
SummationResult CreateSummation(xla::XlaOp input,
  ...
  result.result = xla::Reduce(
      input, init_value,
      compiler::XlaHelpers::CreateAddComputation(shape.element_type()),
      dimensions);
  if (scale) {
  ...
  if (keep_reduced_dimensions) {
    result.result = compiler::XlaHelpers::DynamicReshape(
        result.result, result.rinfo.new_dimensions);
  }
  return result;
}
...
xla::XlaOp CreateProduct(xla::XlaOp input,
  ...
  xla::XlaOp result = xla::Reduce(
      input, init_value,
      compiler::XlaHelpers::CreateMulComputation(shape.element_type()),
      dimensions);
  if (keep_reduced_dimensions) {
...
xla::XlaOp BuildBinaryCrossEntropy(xla::XlaOp input, xla::XlaOp target,
  ...
  if (weight) {
    ...
    xweight = compiler::XlaHelpers::ScalarBroadcast<float>(1.0, input_shape,
                                                           target.builder());
  }
  xla::XlaOp one = xla::One(input.builder(), input_shape.element_type());
  xla::XlaOp log_bound = compiler::XlaHelpers::ScalarValue(
...
xla::XlaOp BuildBinaryCrossEntropyBackward(
  ...
  xla::XlaOp one = xla::One(input.builder(), input_shape.element_type());
  xla::XlaOp epsilon = compiler::XlaHelpers::ScalarValue(
...
xla::XlaOp BuildMseLoss(xla::XlaOp input, xla::XlaOp target,
  ...
  if (reduction == ReductionMode::kMean) {
    ...
    if (num_elements == 0) {
      ...
      xla::XlaOp scale_value = compiler::XlaHelpers::ScalarValue<double>(
...
xla::XlaOp BuildMseLossBackward(xla::XlaOp grad_output, xla::XlaOp input,
                                xla::XlaOp target, ReductionMode reduction) {
  const xla::Shape& input_shape = compiler::XlaHelpers::ShapeOfXlaOp(input);
  xla::XlaOp two = compiler::XlaHelpers::ScalarValue<double>(
...
xla::XlaOp BuildMaxInDims(xla::XlaOp input,
  ...
  xla::XlaOp result = xla::Reduce(
      input, init_value,
      compiler::XlaHelpers::CreateMaxComputation(shape.element_type()),
      dimensions);
  if (keep_reduced_dimensions) {
    result = compiler::XlaHelpers::DynamicReshape(result, rinfo.new_dimensions);
  }
...
xla::XlaOp BuildArgMax(xla::XlaOp input, xla::int64 dim, bool keepdim) {
  ...
  if (dim < 0) {
    dim = 0;
    operand = compiler::XlaHelpers::DynamicReshape(
        operand, {xla::ShapeUtil::ElementsIn(*shape)});
    shape = &compiler::XlaHelpers::ShapeOfXlaOp(operand);
  }
  xla::XlaOp result =
      xla::ArgMaxTwoPass(operand,
                         xla_backend::GetDevicePrimitiveType(
                             xla::PrimitiveType::S64, /*device=*/nullptr),
                         dim,
                         /*tie_low=*/true);
  if (keepdim) {
    auto dimensions = xla::util::ToVector<xla::int64>(shape->dimensions());
    dimensions[dim] = 1;
    result = compiler::XlaHelpers::DynamicReshape(result, dimensions);
  }
  return result;
}
