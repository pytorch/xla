...
xla::XlaOp ComputeNoOverlapMaxPoolIndices(
  ...
  const xla::Shape& padded_input_shape =
      compiler::XlaHelpers::ShapeOfXlaOp(padded_input);
...
PoolSliceIndices ComputeSliceIndices(
  ...
  xla::PrimitiveType scalar_type =
      compiler::XlaHelpers::TypeOfXlaOp(linear_index);
  std::vector<xla::int64> strides =
      xla_backend::ComputeArrayStrides(dimensions);
  PoolSliceIndices indices;
  xla::XlaOp current_index = linear_index;
  for (size_t i = 0; i < dimensions.size(); ++i) {
    xla::XlaOp dim_stride = compiler::XlaHelpers::ScalarValue(
        strides[i], scalar_type, linear_index.builder());
    xla::XlaOp wnd_stride = compiler::XlaHelpers::ScalarValue(
...
xla::XlaOp ComputeMaxPoolIndices(
  ...
  const xla::Shape& pool_result_shape =
      compiler::XlaHelpers::ShapeOfXlaOp(pool_result);
  ...
  size_t limit_id = initial_values.append(compiler::XlaHelpers::ScalarValue(
...
MaxPoolResult BuildMaxPoolNd(xla::XlaOp input, xla::int64 spatial_dim_count,
  ...
  const xla::Shape& input_shape =
      compiler::XlaHelpers::ShapeOfXlaOp(batch_input_info.batch_input);
  xla::XlaOp init_value = xla::MinValue(builder, input_shape.element_type());
...
xla::XlaOp BuildMaxUnpoolNd(const Device& device, xla::XlaOp input,
  ...
  xla::XlaOp flat_input =
      compiler::XlaHelpers::FlattenDimRange(input, 2, output_size.size());
  xla::XlaOp flat_indices =
      compiler::XlaHelpers::FlattenDimRange(indices, 2, output_size.size());

  auto combiner_fn = [](xla::XlaOp x, xla::XlaOp y) -> xla::XlaOp {
    return xla::Max(x, y);
  };
  compiler::ScatterOptions options(combiner_fn);
  ...
  result_sizes.insert(result_sizes.end(), output_size.begin(),
                      output_size.end());
  return compiler::XlaHelpers::DynamicReshape(result, result_sizes);
}
...
xla::XlaOp BuildMaxUnpoolNdBackward(xla::XlaOp grad_output, xla::XlaOp input,
  ...
  xla::XlaOp flat_grad_output =
      compiler::XlaHelpers::FlattenDimRange(grad_output, 2, output_size.size());
  xla::XlaOp flat_indices =
      compiler::XlaHelpers::FlattenDimRange(indices, 2, output_size.size());
  ...
  return compiler::XlaHelpers::DynamicReshapeAs(gather_result, input_shape);
}
...
xla::XlaOp BuildAdaptiveAvgPoolBackward(xla::XlaOp out_backprop,
  ...
  const xla::Shape& out_backprop_shape =
      compiler::XlaHelpers::ShapeOfXlaOp(batch_out_backprop_info.batch_input);
  ...
  auto gradients_size = compiler::XlaHelpers::SizesOfXlaOp(input);
...
xla::XlaOp BuildAdaptiveAvgPool3dBackward(xla::XlaOp out_backprop,
                                          xla::XlaOp input) {
  const xla::Shape& gradients_shape = compiler::XlaHelpers::ShapeOfXlaOp(input);
