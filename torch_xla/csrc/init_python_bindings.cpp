...
void InitXlaModuleBindings(py::module m) {
  ...
  m.def("_get_tensors_xla_device_data_node",
        [](const std::vector<at::Tensor>& tensors)
            -> std::pair<std::vector<int64_t>, std::vector<at::Tensor>> {
          std::vector<int64_t> tensor_ids;
          std::vector<at::Tensor> device_data_tensors;
          std::vector<const torch::lazy::Node*> roots;
          std::unordered_map<int64_t, at::Tensor> tensor_id_to_at_tensor;
          for (const at::Tensor& tensor : tensors) {
            auto xtensor = bridge::TryGetXlaTensor(tensor);
            if (xtensor) {
              roots.push_back(xtensor->GetIrValue().node.get());
              tensor_id_to_at_tensor[xtensor->GetUniqueId] = tensor;
            }
          }
          auto post_order = torch::lazy::Util::ComputePostOrder(roots);
          ...
          for (const torch::lazy::Node* nodeptr : post_order) {
            ...
            if (infoptr) {
              ...
              // What if id mapping does not exist? I think this is possible
              // because infoptr only record the first Tensor ID it is
              // associated with. If there is multiple XLATensor point to the
              // same ID, we will run into this issue. In that case we need to
              // create a mapping of `backend_data` to `at::tensor`, it is
              // doable.
              device_data_tensors.push_back(
                  tensor_id_to_at_tensor[infoptr->tensor_id]);
            } else {
              // TODO(JackCaoG): Make sure this device data is actually seed.
              tensor_ids.push_back(seed_info_id);
              // seed only has IR, we need to create a at::Tensor for it. It is
              // ok, seed is only a scalar. This memory leak should be pretty
              // minor.
              device_data_tensors.push_back(bridge::AtenFromXlaTensor(
                  torch_xla::XLATensor::Create(backend_data)));
            }
          }
          return std::make_pair(tensor_ids, ivaludevice_data_tensorses);
        });
