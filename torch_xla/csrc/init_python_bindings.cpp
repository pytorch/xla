 ...
 *
 * This macro should be used at every Python API entry-point, so that internal
 * `TORCH_WARN()` uses are translated to Python warnings.
 *
 * Even though it's a PyTorch dependency, it doesn't rely on PyTorch specific
 * code, i.e. it's possible to implement it ourselves in this project.
 */
#define TORCH_XLA_PYTHON_HEADER torch::PyWarningHandler ___handler;
...
void BuildLoweringContextSubmodule(py::module* m) {
  ...
  py::class_<PyLoweringContext, std::unique_ptr<PyLoweringContext>>(
      ...
      .def("get_name_string", [](PyLoweringContext& context) -> std::string {
        TORCH_XLA_PYTHON_HEADER
...
void InitXlaModuleBindings(py::module m) {
  ...
  m.def("_xla_get_tensor_view_alias_id", [](const at::Tensor& tensor) {
    TORCH_XLA_PYTHON_HEADER;
    return GetTensorViewAliasId(tensor);
  });
  m.def("_xla_get_tensor_id", [](const at::Tensor& tensor) {
    TORCH_XLA_PYTHON_HEADER;
  ...
  m.def("_xla_get_spmd_config_is_locked", []() {
    TORCH_XLA_PYTHON_HEADER;
  ...
  m.def("_init_computation_client", []() {
    TORCH_XLA_PYTHON_HEADER;
  ...
  m.def("_xla_get_num_cached_compilation_graph", []() -> int64_t {
    TORCH_XLA_PYTHON_HEADER;
  ...
  m.def(
      ...
      py::arg("devices"));
  m.def("_get_executed_fallback_ops", []() {
    TORCH_XLA_PYTHON_HEADER;
  ...
  m.def("_xla_memory_info", [](const std::string& device) {
    TORCH_XLA_PYTHON_HEADER;
  ...
  m.def("_xla_op_builder", [](op_builder::OpPtr op) {
    TORCH_XLA_PYTHON_HEADER;
    return op->builder;
  });
  ...
  m.def("_get_seed_info_id", []() -> int64_t {
    TORCH_XLA_PYTHON_HEADER;
    return seed_info_id;
  });
