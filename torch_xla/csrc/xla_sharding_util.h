...
class ShardingUtil {
 public:
  ...
  // The TensorShardingStore holds per tensor sharding information within
  ...
  class TensorShardingStore {
   public:
    ...
    static void Reset() { tensor_sharding_map_.clear(); }
  ...
  // Shard data nodes with `sharding_spec`. Input data nodes require re-sharding
  // if the device data is replicated by the compiler after executing a
  // computation. This function is intended to be used when there is a sharding
  // annotation attached to the tensor holding the `ir_value`.
  ...
  // evenly partitionable, unless `padded` is set. the returned tensors will be
  // in 1:1 correspondence with the `devices` vector, so the `i`th result will
  // belong on the `i`th device.
