...
static std::string StringifyBroadcastDimensions(std::vector<int64_t> broadcast_dims) {
...
static xla::XlaOp BuildBroadCastForReducedLogits(
  xla::XlaOp reduced_logits, const xla::Shape& logits_shape, int dim) {
  // Suppose BS is unbounded.
  xla::XlaOp dynamic_dim_tensor = xla::Reshape(xla::GetDimensionSize(reduced_logits, 0), {1});
  std::vector<int32_t> static_input_dims_vec(logits_shape.dimensions().begin() + 1, logits_shape.dimensions().end());
  xla::XlaOp static_input_dims = xla::ConstantR1(reduced_logits.builder(), absl::Span<const int32_t>(static_input_dims_vec));
  xla::XlaOp final_broadcast_dimensions = xla::ConcatInDim(reduced_logits.builder(), {dynamic_dim_tensor, static_input_dims}, 0);
  ...
  return xla::CustomCall(
      ...
      /*operands=*/{reduced_logits, final_broadcast_dimensions}, /*shape*/ logits_shape,
...
SoftMaxPartials LogSoftmaxPartials(xla::XlaOp logits, int64_t dim) {
  ...
  std::cout << "check logits_shape shape: " << ShapeHelper::ShapeOfXlaOp(logits) << std::endl;
  std::vector<int64_t> broadcast_dimensions =
      BroadcastDimensions(logits_shape.rank(), dim);
  std::cout << "check broadcast_dimensions: " << broadcast_dimensions << std::endl;
  ...
  xla::XlaOp broadcasted_logits_max = BuildBroadCastForReducedLogits(logits_max, logits_shape, dim);
  xla::XlaOp shifted_logits =
      xla::Sub(logits, broadcasted_logits_max);
  std::cout << "check shifted_logits shape: " << ShapeHelper::ShapeOfXlaOp(shifted_logits) << std::endl;
  xla::XlaOp exp_shifted = xla::Exp(shifted_logits);
  std::cout << "check exp_shifted shape: " << ShapeHelper::ShapeOfXlaOp(exp_shifted) << std::endl;
  xla::XlaOp init_value = xla::Zero(builder, logits_shape.element_type());
  std::cout << "check init_value shape: " << ShapeHelper::ShapeOfXlaOp(init_value) << std::endl;
  xla::XlaOp reduce = xla::Reduce(
      exp_shifted, init_value,
      XlaHelpers::CreateAddComputation(logits_shape.element_type()), {dim});
  std::cout << "check reduce shape: " << ShapeHelper::ShapeOfXlaOp(reduce) << std::endl;
...
xla::XlaOp BuildSoftmax(xla::XlaOp logits, int64_t dim) {
  SoftMaxPartials parts = LogSoftmaxPartials(logits, dim);
  std::cout << "in BuildSoftmax" << std::endl;
  std::cout << "check reduce shape: " << ShapeHelper::ShapeOfXlaOp(parts.reduce) << std::endl;
  std::cout << "check exp_shifted shape: " << ShapeHelper::ShapeOfXlaOp(parts.exp_shifted) << std::endl;
