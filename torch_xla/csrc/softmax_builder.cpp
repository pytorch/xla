...
static xla::XlaOp BuildBroadcastForReducedLogits(xla::XlaOp reduced_logits,
  ...
  const std::vector<int64_t> logits_sizes(
    logits_shape.dimensions().begin(),
  ...
  for (size_t i = 0; i < logits_sizes.size(); ++i) {
    if (logits_sizes.at(i) == xla::Shape::kUnboundedSize) {
      concat_ops.push_back(
        xla::Reshape(xla::GetDimensionSize(logits, i), {1}));
    } else {
      concat_ops.push_back(xla::ConstantR1(logits.builder(),
                      absl::Span<const int32_t>({static_cast<int32_t>(logits_sizes.at(i))})));
    }
  }
