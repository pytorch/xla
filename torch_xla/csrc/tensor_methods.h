...
absl::StatusOr<std::vector<absl_nonnull XLATensorPtr>> user_computation(
    const std::string& opname, absl::Span<const absl_nonnull XLATensorPtr> inputs,
...
absl::StatusOr<absl_nonnull XLATensorPtr> cat(
    absl::Span<const absl_nonnull XLATensorPtr> tensors, int64_t dim, at::ScalarType dtype);
