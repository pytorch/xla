...
public:
   ...
   *        same tensor set/graph as the previous sync.
   */
   ...
   *        then it shall return true.  Callee should return true a maximum of one
   *        time per pass (per unique HashingState object).
   *
   *        - Graph stabilization is based upon pre-rewrite hashes.
   *        - Graph rewrites should be deterministic for a given pre-rewrite hash.
   *        - It is legal to modify some aspects of SyncTensorCollection, such as
   *          the output tensor indices (in the case that a rewrite alters the
   *          graph outputs)
   *
   * Note: Any second call to this function for the same sync is due to recalculating
   *        the hashes from the postorder.  Ideally, this would only occur the *first time*
   *        that a new hash is considered as "stable" and the "final" hash cached from
   *        that point onwards.
   */
