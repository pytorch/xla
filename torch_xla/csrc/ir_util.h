...
class Util {
 public:
  ...
  using EmissionMap = std::unordered_map<const ir::Node*, EmitStatus>;
  ...
  static std::vector<Value> Clone(absl::Span<const ir::Value> values);
  ...
  // nodes argument.
  static size_t GetGraphSize(absl::Span<const ir::Node* const> nodes);
};
