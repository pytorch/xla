...
ReflectionPad3dBackward::ReflectionPad3dBackward(
    const torch::lazy::Value& grad_output, const torch::lazy::Value& input,
    std::vector<int64_t> padding)
    : XlaNode(torch::lazy::OpKind(at::aten::reflection_pad3d_backward),
              {grad_output, input},
              [&]() { return NodeOutputShape(grad_output, input, padding); },
              /*num_outputs=*/1, torch::lazy::MHash(padding)),
      padding_(std::move(padding)) {}

torch::lazy::NodePtr ReflectionPad3dBackward::Clone(
    torch::lazy::OpList operands) const {
  return torch::lazy::MakeNode<ReflectionPad3dBackward>(
      operands.at(0), operands.at(1), padding_);
}

XlaOpVector ReflectionPad3dBackward::Lower(LoweringContext* loctx) const {
...
std::string ReflectionPad3dBackward::ToString() const {
  std::stringstream ss;
