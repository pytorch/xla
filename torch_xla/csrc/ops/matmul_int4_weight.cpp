...
static xla::Shape NodeOutputShape(const torch::lazy::Value& lhs,
...
CastInt4::CastInt4(const torch::lazy::Value& lhs,
                                         const torch::lazy::Value& rhs,
                                         const std::vector<int>& int4_vals)
    : XlaNode(xla_cast_int4, {lhs, rhs}, NodeOutputShape(lhs, rhs),
              /*num_outputs=*/1, torch::lazy::MHash(int4_vals)),
      int4_vals_(int4_vals) {}

torch::lazy::NodePtr CastInt4::Clone(
    torch::lazy::OpList operands) const {
  return torch::lazy::MakeNode<CastInt4>(operands.at(0),
                                                    operands.at(1), int4_vals_);
}
...
XlaOpVector CastInt4::Lower(LoweringContext* loctx) const {
  ...
  // std::vector<int64_t> new_dims(lhs_shape.dimensions().begin(),
  // lhs_shape.dimensions().end()); new_dims.insert(new_dims.begin(), 1);
  // xla::XlaOp lhs_expanded = xla::Reshape(lhs, new_dims);
  // absl::Span<const int64_t>(new_dims.begin(), new_dims.end()));

  // xla::XlaOp weight_4bit = xla::BitcastConvertType(rhs,
  // xla::PrimitiveType::S4); xla::XlaOp weight_4bit_flattened =
  // xla::Collapse(weight_4bit, {1, 2});

  // std::vector<int> values_int8 = {-1, -1, -1, -1, -1, -1, -1, -1};
  std::vector<xla::s4> values(int4_vals_.begin(), int4_vals_.end());
  // const auto literal = xla::LiteralUtil::CreateR2<xla::s4>({{xla::s4(-1),
  // xla::s4(-1), xla::s4(-1), xla::s4(-1)}, {xla::s4(-1), xla::s4(-1),
  // xla::s4(-1), xla::s4(-1)}});
  const auto literal =
      xla::LiteralUtil::CreateR1(absl::Span<const xla::s4>(values));
  // auto reshaped_literal = literal.Reshape({rhs_shape.dimensions()[0],
  // rhs_shape.dimensions()[1]});
  auto reshaped_literal = literal.Reshape(rhs_shape.dimensions());

  return ReturnOp(xla::ConstantLiteral(lhs.builder(), reshaped_literal.value()),
                  loctx);
  ...
  // xla::XlaOp dot = xla::DotGeneral(lhs, xla::ConstantLiteral(lhs.builder(),
  // reshaped_literal.value()), dot_dnums); return ReturnOp(dot, loctx);
}
