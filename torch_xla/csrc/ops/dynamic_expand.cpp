...
DynamicExpand::DynamicExpand(const torch::lazy::Value& input,
                               ...
                               int src_index, int target_index)
    : XlaNode(
          xla_dynamic_expand, {input, src_tensor},
          NodeOutputShape(input, size, src_tensor, src_index, target_index) /* fix when quant type is added to HLO */,
          /*num_outputs=*/1,
...
torch::lazy::NodePtr DynamicExpand::Clone(torch::lazy::OpList operands) const {
  return torch::lazy::MakeNode<DynamicExpand>(operands.at(0), size_, operands.at(1),
...
XlaOpVector DynamicExpand::Lower(LoweringContext* loctx) const {
  ...
  xla::XlaOp final_broadcast_dimensions = xla::ConcatInDim(loctx->builder(), {dynamic_dim_tensor, static_input_dims}, 0);
  
  // Output shape
  xla::Shape final_shape = ShapeHelper::ShapeOfXlaOp(input);
  final_shape.set_unbounded_dynamic_dimension(target_index_);
  xla::XlaOp result = XlaHelpers::DynamicBroadcastInDim(input, final_shape, final_broadcast_dimensions);
