...
DynamicView::DynamicView(const torch::lazy::Value& input,
                               ...
                               int src_index, int target_index, float mul_scaler)
    : XlaNode(
          xla_dynamic_view, {input, src_tensor},
...
torch::lazy::NodePtr DynamicView::Clone(torch::lazy::OpList operands) const {
  return torch::lazy::MakeNode<DynamicView>(operands.at(0), size_, operands.at(1),
                                               src_index_, target_index_, mul_scaler_);
}
...
XlaOpVector DynamicView::Lower(LoweringContext* loctx) const {
  ...
  xla::XlaOp dynamic_dim_tensor = xla::Reshape(xla::GetDimensionSize(src_tensor, src_index_), {1});
  xla::XlaOp dynamic_dim_scaler = xla::ConstantR1(loctx->builder(), absl::Span<const int32_t>({static_cast<int>(mul_scaler_)}));
  xla::XlaOp dynamic_dim_scaled = xla::Mul(dynamic_dim_tensor, dynamic_dim_scaler);
  ...
  xla::XlaOp static_input_dims = xla::ConstantR1(loctx->builder(), absl::Span<const int32_t>(static_input_dims_vec));
  xla::XlaOp final_broadcast_dimensions = xla::ConcatInDim(loctx->builder(), {dynamic_dim_scaled, static_input_dims}, 0);
  ...
  xla::XlaOp result = xla::CustomCall(
      input.builder(), "mhlo.dynamic_reshape",
