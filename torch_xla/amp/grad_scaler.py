...
class GradScaler(object):
  ...
  def _check_scale_growth_tracker(
      self, funcname) -> Tuple[torch.Tensor, torch.Tensor]:
  ...
  def _lazy_init_scale_growth_tracker(self, dev):
    ...
    self._scale = torch.tensor(
        self._init_scale, dtype=torch.float32, device=dev)
    self._growth_tracker = torch.tensor(
      ...
      for device, per_dtype_grads in per_device_and_dtype_grads.items():
        for grads in per_dtype_grads.values():
          torch._amp_foreach_non_finite_check_and_unscale_(
              grads, per_device_found_inf.get(device),
  ...
  def step(self, optimizer, *args, **kwargs):
    ...
    if (hasattr(optimizer, "_step_supports_amp_scaling") and
    ...
    # call mark_step before v.item() to make sure the gradients could be reused in optimizer.step
    xm.mark_step()
    if not sum(
  ...
  def update(self, new_scale=None):
    ...
    if new_scale is not None:
      ...
      if isinstance(new_scale, float):
        self._scale = torch.tensor(
            new_scale, dtype=torch.float32, device=_scale.device)
      else:
        reason = "new_scale should be a float or a 1-element torch.cuda.FloatTensor with requires_grad=False."
        assert isinstance(
            new_scale,
            torch.cuda.FloatTensor), reason  # type: ignore[attr-defined]
      ...
      self._scale = torch._amp_update_scale(_growth_tracker, _scale,
                                            found_inf_combined,
                                            self._growth_factor,
  ...
  def _check_inf_per_device(self, optimizer):
    ...
    dummy_inv_scale = torch.tensor(
        1.0, dtype=torch.float32, device=_scale.device)
    found_inf = torch.tensor(0.0, dtype=torch.float32, device=_scale.device)
