...
def convert_range(range_spec):
    try:
        ...
        raise e
    if len(lowerupper) > 2:
        raise ValueError(f"ERROR: Range specs in NEURON_RT_VISIBLE_CORES should be of "
                         + f"the form <int> or <lower int>-<upper int> (got {range_spec})")
    if len(lowerupper) == 2:
        if lowerupper[0] > lowerupper[1]:
            raise ValueError(f"ERROR: Range specs in NEURON_RT_VISIBLE_CORES should "
    ...
    if range_list:
        ...
        for i in range_list:
            ...
            if (set(cores_list) & set(new)) != set():
                raise ValueError("ERROR: Please ensure the ranges in NEURON_RT_VISIBLE_CORES are mutually exclusive.")
...
def remap_visible_cores(local_rank, local_world_size):
    ...
    if count <= 1 and local_world_size == 1:
        ...
        raise ValueError(f"LOCAL_WORLD_SIZE (torchrun) or PJRT_LOCAL_PROCESS_COUNT (xmp.spawn) value of {local_world_size} is " +
                         f"not equal to count {count} of cores in NEURON_RT_VISIBLE_CORES {cores_list}")
    elif local_rank >= count:
        raise ValueError(f"LOCAL_RANK (torchrun) or PJRT_LOCAL_PROCESS_RANK (xmp.spawn) value of {local_rank} is higher than " +
                         f"count {count} of cores in NEURON_RT_VISIBLE_CORES {cores_list}")
    else:
