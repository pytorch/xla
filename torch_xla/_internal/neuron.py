    ...
    if os.environ.get('NEURON_RT_ROOT_COMM_ID', None) is None:
        ...
        if is_ipv6:
            ...
            if not root_addr.endswith("]"):
                ...
                modified = True
            if modified:
                logger.warning("IPv6 address detected for MASTER_ADDR and missing brackets added: {}".format(root_addr))
...
def configure_pjrt_environment():
    ...
    if os.environ.get('XLA_USE_SPMD', '0') != '1' and os.environ.get(
            'PJRT_DEVICE', None) == 'NEURON' and torch.distributed.is_torchelastic_launched():
        ...
        if 'RANK' not in os.environ:
            logger.warning(
                'RANK environment variable is not set, defaulting to 0.')
