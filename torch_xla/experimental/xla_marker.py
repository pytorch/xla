...
XLA_LIB.define(
    "mark_tensor(Tensor x, str name, int pos, str id, bool is_input, str? attr=None) -> Tensor"
)
...
def serialize_composite_attr(attr: Dict) -> str:
  if attr is None:
    return None
...
@torchdynamo.assume_constant_result
def deserialize_composite_attr(attr: str) -> Dict:
  if attr is None:
    return None
...
def mark_tensor_xla(x: torch.Tensor,
  ...
  Args:
      ...
      attr: str - Attribute of the pattern, it will be passed down to the attribute field
                  in the stablehlo composite. Attr must be a str generated by 
