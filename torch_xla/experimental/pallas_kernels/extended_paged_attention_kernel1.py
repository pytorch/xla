# v1 of the extended paged attention kernel.  # do repeat_interleave
...
def _flash_attention(
    q_head_idx_for_cur_kv_head,  # scalar, ranges from 0 to num_query_heads_per_kv_head
  ...
  def start_new_sequence():
    ...
    l_scratch_ref[q_head_idx_for_cur_kv_head] = jnp.zeros(l_scratch_ref[q_head_idx_for_cur_kv_head].shape, jnp.float32)
    m_scratch_ref[q_head_idx_for_cur_kv_head] = jnp.full(m_scratch_ref[q_head_idx_for_cur_kv_head].shape, -jnp.inf, jnp.float32)
    acc_scratch_ref[q_head_idx_for_cur_kv_head] = jnp.zeros(acc_scratch_ref[q_head_idx_for_cur_kv_head].shape, jnp.float32)

  m_prev = m_scratch_ref[q_head_idx_for_cur_kv_head]
  l_prev = l_scratch_ref[q_head_idx_for_cur_kv_head]
  q = q_ref[0, q_head_idx_for_cur_kv_head, :, :].astype(jnp.float32)  # [block_q, head_dim]
  ...
  l_scratch_ref[q_head_idx_for_cur_kv_head] = l_next
  m_scratch_ref[q_head_idx_for_cur_kv_head] = m_next

  l_next_inv_safe = jnp.where(l_next == 0.0, 1.0, 1.0 / l_next)
  acc_scratch_ref[q_head_idx_for_cur_kv_head] *= l_broadcast(l_corr * l_next_inv_safe)
  ...
  acc_scratch_ref[q_head_idx_for_cur_kv_head] += o_curr * l_broadcast(l_next_inv_safe)
  ...
  # def store_output():
  o_ref[0, q_head_idx_for_cur_kv_head] = acc_scratch_ref[q_head_idx_for_cur_kv_head].astype(o_ref.dtype)
  l_ref[0, q_head_idx_for_cur_kv_head] = l_scratch_ref[q_head_idx_for_cur_kv_head].astype(l_ref.dtype)
  m_ref[0, q_head_idx_for_cur_kv_head] = m_scratch_ref[q_head_idx_for_cur_kv_head].astype(m_ref.dtype)
...
def paged_flash_attention_kernel(
  ...
  def get_kv_and_run_flash_attention():
    ...
    pl.debug_print('xw32 line379 For q_blk_idx={}, calculated next_b={}, next_kv_head_idx={}, next_kv_blk_idx={}', q_blk_idx, next_b, next_kv_head_idx, next_kv_blk_idx)
    ...
    pl.debug_print('xw32 line385 Retriving kv pages corresponding to b={}, kv_head_idx={}, q_blk_idx={}, kv_blk_idx={}', b, kv_head_idx, q_blk_idx, kv_blk_idx)
    async_copy_k, async_copy_v = create_kv_async_copy_descriptors(
      b, kv_head_idx, kv_blk_idx, buffer_index
    )
...
def paged_attention(
  ...
  l_scratch = pltpu.VMEM((num_q_heads_per_kv_head, num_queries_per_compute_block, MIN_BLOCK_SIZE),
                          jnp.float32)
  m_scratch = pltpu.VMEM((num_q_heads_per_kv_head, num_queries_per_compute_block, MIN_BLOCK_SIZE),
                          jnp.float32)
  acc_scratch = pltpu.VMEM((num_q_heads_per_kv_head, num_queries_per_compute_block, head_dim),
                          jnp.float32)
  scratch_shapes = [
      pltpu.VMEM(
          (
