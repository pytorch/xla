...
# Improved by
# 1. Only write to HBM at the last iteration. (#8393)
# 2. Skip the computation if the current block to above the causal mask diagonal. #8448
...
def paged_attention(
  ...
  #   raise ValueError(
  #       "pages_per_sequence must be divisible by num_kv_pages_per_compute_block. Got"
  ...
  grid = (
      ...
      pl.cdiv(pages_per_sequence,num_kv_pages_per_compute_block),  # how many compute blocks we need to loop the kv_len
  )
