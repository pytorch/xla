...
def _flash_attention(
    q_head_idx_for_cur_k_head,  # scalar, ranges from 0 to num_query_heads_per_kv_head
  ...
  q = q_ref[0, q_head_idx_for_cur_k_head, :, :].astype(jnp.float32)  # [block_q, head_dim]
  ...
  o_ref[0, q_head_idx_for_cur_k_head] = acc_scratch_ref[...].astype(o_ref.dtype)
  l_ref[0, q_head_idx_for_cur_k_head] = l_scratch_ref[...].astype(l_ref.dtype)
  m_ref[0, q_head_idx_for_cur_k_head] = m_scratch_ref[...].astype(m_ref.dtype)
...
def paged_attention(
  ...
  Args:
    ...
    num_queries_per_compute_block: how many queries to be processed in one block in the query dimension.
