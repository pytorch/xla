...
def make_group_metadata(
  ...
  Returns:
    tuple of:
      ...
      group_ids: A 1d, torch.Tensor with shape [m_tiles + num_groups] and
        torch.int32 dtype. group_ids[i] indicates which group grid index 'i' will
        work on.
      m_tile_ids: A 1d, torch.Tensor with shape [m_tiles + num_groups] and
  ...
  group_ends = np.cumsum(group_sizes.numpy(force=True))
  group_offsets = np.concatenate([np.zeros(1, dtype=np.int32), group_ends])
  ...
  rounded_group_ends = ((group_ends + tm - 1) // tm * tm).astype(np.int32)

  # (2) Round the group_starts down to the nearest multiple of 'tm'.
  group_starts = np.concatenate(
      [np.zeros(1, dtype=np.int32), group_ends[:-1]]
  )
  ...
  rounded_group_sizes = np.where(group_sizes.numpy(force=True) == 0, 0, rounded_group_sizes)
  ...
  if visit_empty_groups:
    ...
    group_tiles = np.where(group_sizes.numpy(force=True) == 0, 1, group_tiles)
  ...
  # TODO(apaszke, enriqueps): This is unnecessary if visit_empty_groups=False
  # before_start_group = group_ids < start_group
  # after_end_group = group_ids > end_group
  # group_ids = np.where(before_start_group, start_group, group_ids)
  # group_ids = np.where(after_end_group, end_group, group_ids)
  # group_ids -= start_group
  ...
  partial_tile_mask = np.logical_or(
      (group_offsets[:-1] % tm) == 0, group_sizes.numpy(force=True) == 0
  )
  ...
  if visit_empty_groups:
    partial_tile_mask = np.where(group_sizes == 0, 0, partial_tile_mask)

  partial_tile_ids = np.where(
      partial_tile_mask, tiles_m, group_offsets[:-1] // tm
  )

  tile_visits = (
      np.histogram(partial_tile_ids, bins=tiles_m, range=(0, tiles_m - 1))[0]
      + 1
  )

  # Create the m-dimension tile ids for each grid index based on the visit
  # counts for each tile.
  # m_tile_ids = np .arange(tiles_m, dtype=np.int32).repeat(
  #     tile_visits.astype(np.int32),
  #     #total_repeat_length=tiles_m + num_groups,
  # )
  ...
  group_ids = np.roll(group_ids, shift=-first_tile_in_shard, axis=0)
  m_tile_ids = np.roll(m_tile_ids, shift=-first_tile_in_shard, axis=0)

  # Calculate the number of tiles we need to compute for our shard.
  #
  # Remove tile visits that belong to a group not in our shard.
  iota = np.arange(num_groups, dtype=np.int32)
  active_group_mask = np.logical_and(iota <= end_group, iota >= start_group)
  group_tiles = np.where(active_group_mask, group_tiles, 0)
  num_tiles = group_tiles.sum()
  return (torch.from_numpy(group_offsets).to(torch.int32).to("xla"), \
          torch.from_numpy(group_ids).to("xla"), \
          torch.from_numpy(m_tile_ids).to("xla")), \
          torch.from_numpy(np.array([num_tiles])).to(torch.int32).to("xla")
...
def gmm(
  ...
  group_metadata, num_active_tiles = make_group_metadata(  # pylint: disable=unbalanced-tuple-unpacking
      ...
      m=m,
      tm=tm,
      start_group=group_offset[0].numpy(force=True),
  ...
  out = torch_xla._XLAC._xla_tpu_custom_call([num_active_tiles, group_metadata[0], group_metadata[1], group_metadata[2], group_offset, lhs, rhs], payload, output_shapes, output_dtypes)
  # print("out", out)
