...
def pt2e_reference_model_to_stablehlo(m: GraphModule,
  ...
  if emit_bytecode:
    return stablehlo_model.get_stablehlo_bytecode('forward')
  else:
    return stablehlo_model.get_stablehlo_text('forward')
...
def pt2e_reference_model_to_tf_saved_model(m: GraphModule, args: Tuple[Any,
  ...
  options = stablehlo.StableHLOExportOptions(override_tracing_arguments=args)
  stablehlo_model = stablehlo.exported_program_to_stablehlo(
