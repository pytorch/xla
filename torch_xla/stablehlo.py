...
from torch.fx import _pytree as fx_pytree, subgraph_rewriter
...
from torch_xla.experimental.stablehlo_custom_call import (
    extract_custom_call_outputs_shape_dtype,
...
class StableHLOGraphModule:
    ...
    def evaluate(self, method_name, args):
        ...
        call_args = [
            torch.from_numpy(x) if isinstance(x, np.ndarray) else x for x in call_args
        ]
...
class XLAExportInterpreter(torch.fx.Interpreter):
    ...
    def __init__(self, module, device, custom_ops_allowed_in_graph, gm_serializer):
        self._device = device
        ...
        if hasattr(target, "_schema"):
            ...
            with torch.utils._python_dispatch._disable_current_modes():
                ...
                args = tuple(
                    map(
                        lambda arg_spec: (
                            torch.tensor(arg_spec[0])
                            if isinstance(arg_spec[0], float)
                            and type(arg_spec[1].type) == torch.TensorType
                            else arg_spec[0]
                        ),
                        args_and_specs,
                    )
    ...
    def run_node(self, n) -> Any:
        ...
        if self.gm_serializer is not None:
            ...
            pytree.tree_map_only(
                torch.Tensor, lambda x: write_mlir_debuginfo(x, node_metadata), res
            )
        return res
...
def _exported_program_to_stablehlo_bundle(
    exported_model, options
    ...
    _flat_input_args = pytree.tree_map_only(
        torch.Tensor, lambda x: x.to(device=device), _flat_input_args
    )
    ...
    if options.export_node_metadata:
        gm_serializer = GraphModuleSerializer(
            exported_model.graph_signature, exported_model.module_call_graph
        )
        os.environ["XLA_HLO_DEBUG"] = "1"
    else:
        gm_serializer = None

    xla_interpreter = XLAExportInterpreter(
        exported_model.graph_module,
        device,
    ...
    for hlo_input_pos, (tensor_id, tensor_value) in enumerate(
        ...
        if tensor_id in input_ids:  # this is input
            ...
            location = InputLocation.parameter(name=tensor_id_to_state_name[tensor_id])
    ...
    output_signature = [
        VariableSignature(
            shape=list(tensor.shape), dtype=str(tensor.dtype).replace("torch.", "")
        )
        for tensor in res
    ]
    ...
    meta = StableHLOFunctionMeta(
        ...
        output_pytree_spec=pytree.treespec_dumps(exported_model.call_spec.out_spec),
    )
...
def exported_program_to_stablehlo(
    exported_model: "ExportedProgram", options: Optional[StableHLOExportOptions] = None
