...
class ExperimentLoader:
    ...
    def is_available(self, experiment_config):
        if experiment_config["dynamo"] and experiment_config[
            "dynamo"
        ] not in dynamo.list_backends(exclude_tags=()):
            return False
        if experiment_config["dynamo"] == "inductor" and not (
            experiment_config["accelerator"] == "cuda" and not experiment_config["xla"]
        ):
            return False
        ...
        if experiment_config["dynamo"] == "openxla" and not (
            ...
            return False
        if experiment_config["xla"] and not is_xla_device_available(
        ...
        if experiment_config["accelerator"] == "tpu" and not experiment_config["xla"]:
            return False
        if (
            experiment_config["accelerator"] == "cuda"
            and not experiment_config["xla"]
...
class BenchmarkExperiment:
    def __init__(self, experiment_name, accelerator, xla, dynamo, test, batch_size):
