...
DENY_LIST = {
    ...
    "mobilenet_v2_quantized_qat": [
        {"test": "eval", "accelerator": "cuda"},  # not implemented
        {"test": "eval", "accelerator": "tpu"},
    ],  # not implemented
...
class TorchBenchModel(BenchmarkModel):
    ...
    def set_up(self):
        ...
        cant_change_batch_size = not getattr(
            benchmark_cls, "ALLOW_CUSTOMIZE_BSIZE", True
        )
        ...
        if self.benchmark_experiment.accelerator == "cpu":
            ...
            self.benchmark_experiment.accelerator == "cuda"
            and not self.benchmark_experiment.xla
        ):
            device = "cuda"
        else:
        ...
        if self.model_name == "yolov3":
            self.example_inputs = (
                torch.rand(self.benchmark_experiment.batch_size, 3, 384, 512),
            )
        if (
            self.benchmark_experiment.test == "train"
            and self.model_name in DETECTRON2_MODELS
        ):
