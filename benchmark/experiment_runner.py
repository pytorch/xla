...
class ExperimentRunner:
    ...
    def run(self):
        if self._args.experiment_config and self._args.model_config:
            ...
            if os.path.exists(self.output_file):
                if self._args.no_resume:
                    ...
                    for jsonline in jsonlines:
                        ...
                        finished_experiments.add(
                            "-".join(
                                str(item)
                                for item in (
                                    list(tmp["model"].values())
            ...
            for model_config in tqdm(
                model_configs, desc="model configs", disable=not self._args.progress_bar
            ):
                for experiment_config in experiment_configs:
                    ...
                    if (
                        "-".join(
                            str(item)
                            for item in (
                                list(dummy_benchmark_model.to_dict().values())
                        ...
                        continue
                    if self.model_loader.is_compatible(
                        dummy_benchmark_model, dummy_benchmark_experiment
                    ):
                        try:
    ...
    def run_single_experiment(self, experiment_config, model_config):
        benchmark_experiment = self.experiment_loader.load_experiment(experiment_config)
        ...
        with benchmark_model.pick_grad():
            ...
            for i in range(self._args.repeat):
                run_metrics, output = self.timed_run(
    ...
    def save_results(self, benchmark_experiment, benchmark_model, metrics, outputs):
        if self._args.save_output and outputs is not None:
            outputs_file_name = (
                f"{benchmark_model.filename_str}-{benchmark_experiment.filename_str}.pt"
            )
    ...
    def timed_run(self, benchmark_experiment, benchmark_model):
        ...
        inputs_list = self.prepare_inputs(
            benchmark_model.example_inputs, self._args.randomize_input
        )
...
def main():
    ...
    # Set filter based on tier
    if args.filter_by_tier is not None:
        args.filter = []
        args.exclude = [r"^$"]
        for tier in args.filter_by_tier:
            if tier in FILTER_BY_TIER:
