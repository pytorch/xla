...
def get_build_version(git_head_sha):
  version = os.getenv('TORCH_XLA_VERSION', '0.1')
  if _check_env_flag('VERSIONED_XLA_BUILD', default='0'):
    try:
      version += '+' + git_head_sha[:7]
    except Exception:
      pass
  return version


def create_version_files(base_dir, version, git_head_sha):
...
git_head_sha = get_git_head_sha(base_dir)
version = get_build_version(git_head_sha)

# Generate version info (torch_xla.__version__)
create_version_files(base_dir, version, git_head_sha)
