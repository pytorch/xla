...
_CTOR_FUNCTIONS = {
    'empty': '.device(xla::NNCComputationClient::HardwareDeviceType())',
    'linspace': '.device(xla::NNCComputationClient::HardwareDeviceType())',
    'logspace': '.device(xla::NNCComputationClient::HardwareDeviceType())',
    'rand': '.device(xla::NNCComputationClient::HardwareDeviceType())',
    'rand_like': '.device(xla::NNCComputationClient::HardwareDeviceType())',
    'randn': '.device(xla::NNCComputationClient::HardwareDeviceType())',
    'randn_like': '.device(xla::NNCComputationClient::HardwareDeviceType())',
    'randint': '.device(xla::NNCComputationClient::HardwareDeviceType())',
    'randint_like': '.device(xla::NNCComputationClient::HardwareDeviceType())',
    'randperm_out': '.device(xla::NNCComputationClient::HardwareDeviceType())',
    'scalar_tensor': '.device(xla::NNCComputationClient::HardwareDeviceType())',
}
...
class TensorFetcher(object):
  ...
  def generate_updates(self):
    ...
    if self.writeable:
      assert self.tensors
      code += '  if (!bridge::IsInteropView({})) {{\n'.format(self.tensors[0])
...
def get_tuple_return(rtype,
  ...
  for i in range(0, n):
    ...
    ttype = types[ti]
    if type_core(ttype) == 'Tensor':
      tuple_var +=  '.contiguous()'
...
def generate_entry_debug_code(t, fname, params, fname_ns=None):
  ...
  if _check_env_flag('DEBUG'):
    ...
    code += '  LTC_VLOG(3) << "XLA {} :"'.format(fname)
    for p in params:
      ptype = param_type(p)
      cptype = type_core(ptype)
      pname = param_name(p)
