...
def generate_return_stmt(t, orig_sig, fname, rname, params, param_vars, ref_param):
  assert isinstance(t, lark.tree.Tree)
  rtype = t.children[0]
...
def get_xla_wrapper(orig_sig, ctx):
  ...
  code += '  {}{}('.format(result_assign, ctx.get_function(fname, xla_ref_param))
...
def generate(args, typedef_file, functions_file):
  ofile = sys.stdout
  if args.output:
    ofile = open(args.output, 'w')

  fndefs = extract_functions(typedef_file)
  print(
      'Extracted {} functions from {}'.format(len(fndefs), typedef_file),
      file=sys.stderr)
  fgens = []
  ctx = Context(functions_file)
  for ts in fndefs:
...
if __name__ == '__main__':
  ...
  arg_parser.add_argument('--typedef', type=str)
  arg_parser.add_argument('--functions', type=str)
  args, files = arg_parser.parse_known_args()
  assert len(files) == 2
  generate(args, files[0], files[1])
