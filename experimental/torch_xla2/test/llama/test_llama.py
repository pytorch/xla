...
class LlamaTest(test_base.TestCase):
  ...
  def test_can_run_exportable(self):
    ...
    def make_cache(args, batch_size):
      ...
      for i in range(args.n_layers):
        ...
        res.append(
          (
            torch.zeros(
              size, dtype=torch.bfloat16 if args.bf16_enable else torch.float
            ),
            torch.zeros(
              size, dtype=torch.bfloat16 if args.bf16_enable else torch.float
            ),
    ...
    sample_inputs = pytree.tree_map_only(
      torch.Tensor, tensor.t2j, sample_input_prefill
    )
    ...
    sample_inputs = pytree.tree_map_only(
      torch.Tensor, tensor.t2j, sample_input_decode
    )
