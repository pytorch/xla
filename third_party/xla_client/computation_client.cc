...
bool IsLocalDevice(const PjrtComputationClient::Worker& worker,
  ...
  std::string mp_device = PjrtComputationClient::GetMultiProcessingDevice();
  if (mp_device.empty()) {
    return true;
  }
  PjrtComputationClient::Device device(mp_device);
...
std::map<std::string, int> BuildDeviceTaskMap(
    const PjrtComputationClient::Options& options) {
  ...
  for (auto& device_pjrt_device : options.global_device_map) {
    PjrtComputationClient::Device global_device(device_pjrt_device.first);
    tensorflow::DeviceNameUtils::ParsedName parsed_device =
        ParsePjrtDevice(device_pjrt_device.second);
    std::string task_device_key =
...
void PopulateLocalDevices(PjrtComputationClient::Options* options) {
  std::string local_worker = sys_util::GetEnvString(env::kEnvLocalWorker, "");
  PjrtComputationClient::Worker worker("", -1);
  if (!local_worker.empty()) {
    worker = PjrtComputationClient::ParseWorker(local_worker);
  }
  ...
  for (auto& device_pjrt_device : options->global_device_map) {
    ...
    options->devices.insert(device_pjrt_device.first);
...
void AddPjrtHostDevices(const std::string& worker_name, int task_no,
  ...
  options->workers_map.emplace(
      PjrtComputationClient::Worker(worker_name, task_no),
  ...
  for (auto& device : devices) {
    ...
    for (int j = 0; j < device.count; ++j, ++device_ordinal) {
      ...
      std::string pjrt_device_name =
          GetPjrtDevicePath(worker_name, task_no, device.tf_name, j);
      options->global_device_map.emplace(device_name, pjrt_device_name);
    }
  }
}

bool ParseEnvBasedTpuClusterConfig(PjrtComputationClient::Options* options) {
  ...
  for (const auto& spec : spec_parts) {
    ...
    AddPjrtHostDevices(host_parts[0], std::stoi(host_parts[1]), host_parts[2],
...
bool ParseMeshConfig(
  ...
  XLA_CHECK(!local_worker_env.empty())
      << "In a mesh client setup the PJRT_DEVICE must be specified";

  PjrtComputationClient::Worker local_worker =
      PjrtComputationClient::ParseWorker(local_worker_env);
  ...
  for (auto& config_worker : config.workers()) {
    PjrtComputationClient::Worker worker(config_worker.name(),
                                        config_worker.task_no());
    options->workers_map.emplace(worker, config_worker.address());

    for (auto& device : config_worker.devices()) {
      PjrtComputationClient::Device local_device(device.local_name());
      options->global_device_map.emplace(
          device.global_name(),
          GetPjrtDevicePath(worker.name, worker.task_no, local_device.kind,
...
bool ParseEnvDeviceCounts(PjrtComputationClient::Options* options) {
  DeviceCountDefaults device_counts;
  ...
  if (device_counts.num_tpus > 0 || device_counts.num_gpus > 0) {
    ...
    AddPjrtHostDevices("localservice", 0, host_port, device_counts,
                      &device_ordinals, options);
  }
  return !options->global_device_map.empty();
}

bool ParseEnvDevices(PjrtComputationClient::Options* options) {
  ...
  if (!device_spec.empty() && !workers_spec.empty()) {
    ...
    for (const auto& name_target : absl::StrSplit(workers_spec, '|')) {
      ...
      options->workers_map.emplace(PjrtComputationClient::ParseWorker(parts[0]),
...
std::unique_ptr<ComputationClient> ComputationClient::Create() {
  ...
  if (sys_util::GetEnvString(env::kEnvPjRtDevice, "") != "") {
    ...
    client = std::unique_ptr<ComputationClient>(
        new PjrtComputationClient(options, std::move(topology_proto)));
  }
...
void ComputationClient::RunLocalService(uint64_t service_port) {
  try {
    PjrtLocalService* service = new PjrtLocalService(
