...
function run_torch_xla_tests() {
  ...
  pushd $XLA_DIR
    ...
    if [ "$USE_COVERAGE" != "0" ]; then
      ...
      # GPU tests
      if [ -x "$(command -v nvidia-smi)" ]; then
        PJRT_DEVICE=GPU GPU_NUM_DEVICES=2 python test/test_train_mp_imagenet_fsdp.py --fake_data --use_nested_fsdp --use_small_fake_sample --num_epochs=1
        PJRT_DEVICE=GPU GPU_NUM_DEVICES=2 python test/test_train_mp_imagenet_fsdp.py --fake_data --auto_wrap_policy type_based --use_small_fake_sample --num_epochs=1
        XLA_DISABLE_FUNCTIONALIZATION=1 PJRT_DEVICE=GPU GPU_NUM_DEVICES=2 python test/test_train_mp_imagenet_fsdp.py --fake_data --use_nested_fsdp --use_small_fake_sample --num_epochs=1
