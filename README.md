...
To start, [you create a Cloud TPU node](https://cloud.google.com/tpu/docs/tutorials/resnet-alpha-py#create_tpu) with the corresponding release you wish to consume (TPU software version: ex. `pytorch-1.6`):
...
2. SSH into the VM and pull a version of the docker image into the VM. The currently available versions are:
    ...
    * `gcr.io/tpu-pytorch/xla:r1.6`: The current stable version.
    ...
    * `gcr.io/tpu-pytorch/xla:nightly_3.6_YYYYMMDD (e.g.: gcr.io/tpu-pytorch/xla:nightly_3.6_20190531)`: The nightly version of the given day. You can replace `3.6` with `3.7` if desired.
    ...
    ```Shell
    (vm)$ docker pull gcr.io/tpu-pytorch/xla:r1.6
    ```
...
3. Where `$TPU_IP_ADDRESS` (e.g.: `10.1.1.2`) is your TPU Internal IP displayed in GCP UI, after pulling the docker image you can either:
    ...
    * Run the container with a single command:
      ```Shell
      (vm)$ docker run --shm-size 16G -e XRT_TPU_CONFIG="tpu_worker;0;$TPU_IP_ADDRESS:8470" gcr.io/tpu-pytorch/xla:r1.6 python /pytorch/xla/test/test_train_mp_mnist.py
      ```

    * Run the script in an interactive shell:
      ```Shell
      (vm)$ docker run -it --shm-size 16G gcr.io/tpu-pytorch/xla:r1.6
...
2. SSH into VM and activate the conda environment you wish to use. Each release (e.g.: `1.5`, `1.6`, `nightly`) is a separate conda environment.
    ...
    (torch-xla-1.6)$ cd /usr/share/torch-xla-1.6/pytorch/xla
    (torch-xla-1.6)$ python test/test_train_mp_mnist.py
    ```
...
```
(torch-xla-1.6)$ python -m torch_xla.distributed.xla_dist --tpu=$TPU_POD_NAME --conda-env=torch-xla-1.6 --env=XLA_USE_BF16=1 -- python /usr/share/torch-xla-1.6/pytorch/xla/test/test_train_mp_imagenet.py --fake_data
```

* Or, to distribute training as a docker container:
```
(torch-xla-1.6)$ python -m torch_xla.distributed.xla_dist --tpu=$TPU_POD_NAME --docker-image=gcr.io/tpu-pytorch/xla:r1.6 --docker-run-flag=--rm=true --docker-run-flag=--shm-size=50GB --env=XLA_USE_BF16=1 -- python /pytorch/xla/test/test_train_mp_imagenet.py --fake_data
```
...
```
(torch-xla-1.6)$ cd /usr/share/torch-xla-1.6/pytorch/xla
(torch-xla-1.6)$ python -m torch_xla.distributed.xla_dist --tpu=$TPU_POD_NAME --vm $VM1 --vm $VM2 --vm $VM3 --vm $VM4 --conda-env=torch-xla-1.6 --env=XLA_USE_BF16=1 -- python test/test_train_imagenet.py --fake_data
```
